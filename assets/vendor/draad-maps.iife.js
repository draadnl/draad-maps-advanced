...
`;class tu extends HTMLElement{#t;#n=null;#e;#i=null;#a=null;#l=null;#r=-1;#s=[];#o=null;#h=null;#d;#u;static get observedAttributes(){return["placeholder","label","search-properties","search-labels","radius","min-chars","debounce","address-filter"]}attributeChangedCallback(e,n,s){n!==s&&this.#n&&(this.#m(),Ut(this.#t),this.#_())}constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#d=`search-input-${Math.random().toString(36).slice(2,9)}`,this.#u=`search-listbox-${Math.random().toString(36).slice(2,9)}`,this.#e=this.#c()}#c(){const e=new CSSStyleSheet;e.replaceSync(Yd),this.#t.adoptedStyleSheets=[e]}get placeholder(){return this.getAttribute("placeholder")||"Zoeken..."}set placeholder(e){this.setAttribute("placeholder",e)}get label(){return this.getAttribute("label")||"Zoek op straat, wijk of stadsdeel"}set label(e){this.setAttribute("label",e)}get searchProperties(){const e=this.getAttribute("search-properties");return e?e.split(",").map(n=>n.trim()).filter(n=>n):null}set searchProperties(e){Array.isArray(e)?this.setAttribute("search-properties",e.join(",")):this.setAttribute("search-properties",e)}get searchLabels(){const e=this.getAttribute("search-labels");return e?e.split(",").map(n=>n.trim()).filter(n=>n):null}set searchLabels(e){Array.isArray(e)?this.setAttribute("search-labels",e.join(",")):this.setAttribute("search-labels",e)}get radius(){const e=this.getAttribute("radius"),n=parseInt(e,10);return Number.isFinite(n)&&n>0?n:1e4}set radius(e){this.setAttribute("radius",String(e))}get minChars(){const e=this.getAttribute("min-chars"),n=parseInt(e,10);return Number.isFinite(n)&&n>=0?n:2}set minChars(e){this.setAttribute("min-chars",String(e))}get debounceDelay(){const e=this.getAttribute("debounce"),n=parseInt(e,10);return Number.isFinite(n)&&n>=0?n:300}set debounceDelay(e){this.setAttribute("debounce",String(e))}get addressFilter(){const e=this.getAttribute("address-filter");return e&&e.trim()?e.trim():null}set addressFilter(e){this.setAttribute("address-filter",e)}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#f(e):e.addEventListener("dm-map:ready",()=>{this.#f(e)},{once:!0}))}async#f(e){this.#n=e,await this.#e,this.#m(),Ut(this.#t),this.#_()}#m(){this.#t.innerHTML=`
		...
		`}#_(){const e=this.#t.querySelector(".search__input"),n=this.#t.querySelector(".search__button"),s=this.#t.querySelector(".search__clear"),a=this.#t.querySelector(".search__listbox");e.addEventListener("input",l=>this.#y(l)),e.addEventListener("keydown",l=>this.#L(l)),e.addEventListener("focus",()=>this.#p()),e.addEventListener("blur",l=>this.#w(l)),n.addEventListener("click",()=>this.#P()),s.addEventListener("click",()=>{e.value="",s.hidden=!0,this.#s=[],this.#k(),e.focus()}),a.addEventListener("mousedown",l=>{l.preventDefault()}),a.addEventListener("click",l=>{const h=l.target.closest('[role="option"]');if(h){const c=parseInt(h.dataset.index,10);this.#H(c)}})}#y(e){const n=e.target.value.trim(),s=this.#t.querySelector(".search__clear");if(s&&(s.hidden=!e.target.value),this.#l&&(clearTimeout(this.#l),this.#l=null),this.#T(),n.length<this.minChars){this.#s=[],this.#k();return}this.#l=setTimeout(()=>{this.#g(n)},this.debounceDelay)}#L(e){const s=!this.#t.querySelector(".search__listbox").hidden;switch(e.key){case"ArrowDown":e.preventDefault(),s&&this.#D(1);break;case"ArrowUp":e.preventDefault(),s&&this.#D(-1);break;case"Enter":if(e.preventDefault(),s&&this.#s.length>0){const a=this.#r>=0?this.#r:0;this.#H(a)}else this.#S();break;case"Escape":e.preventDefault(),this.#k();break;case"Tab":this.#k();break}}#p(){this.#s.length>0&&this.#W()}#w(e){this.#a=setTimeout(()=>{this.#k()},150)}#P(){if(this.#s.length>0){const e=this.#r>=0?this.#r:0;this.#H(e)}else this.#S()}#S(){const n=this.#t.querySelector(".search__input").value.trim();n.length>=this.minChars&&this.#g(n)}async#g(e){this.#i&&this.#i.abort(),this.#i=new AbortController,this.#j(!0);try{const[n,s]=await Promise.all([this.#C(e),this.#N(e,this.#i.signal)]),a=s?.error===!0,l=a?s.results:s;this.#s=[...n.slice(0,5),...l.slice(0,10)],this.#q(a),this.#W();const h=this.#s.length;let c=h>0?`${h} resultaat${h!==1?"en":""} gevonden`:"Geen resultaten gevonden";a&&(c+=". Zoeken in adressen niet beschikbaar"),this.#Z(c)}catch(n){n.name!=="AbortError"&&this.#Z("Er is een fout opgetreden bij het zoeken")}finally{this.#j(!1)}}async#N(e,n){const s=new URL("https://api.pdok.nl/bzk/locatieserver/search/v3_1/free");s.searchParams.set("q",e),s.searchParams.set("rows","10"),s.searchParams.set("fq","type:(provincie OR gemeente OR woonplaats OR wijk OR buurt OR weg OR adres)"),this.addressFilter&&s.searchParams.append("fq",this.addressFilter),s.searchParams.set("fl","id,weergavenaam,type,score,centroide_ll,geometrie_ll");try{const a=await fetch(s.toString(),{signal:n});if(!a.ok)throw new Error(`HTTP ${a.status}`);return((await a.json()).response?.docs||[]).map(c=>({type:"address",label:c.weergavenaam,subtype:c.type,coordinates:this.#x(c.centroide_ll),geometry:c.geometrie_ll?this.#B(c.geometrie_ll):null,bounds:null,raw:c}))}catch(a){if(a.name==="AbortError")throw a;return{error:!0,results:[]}}}#C(e){const n=[],s=e.toLowerCase(),a=this.searchProperties,l=this.#n.querySelectorAll("dm-geojson, dm-wfs");for(const c of l){if(n.length>=5)break;const u=c.data;if(!u?.features)continue;const m=c.geoJsonLayer||c.wfsLayer;if(m)for(const v of u.features){if(n.length>=5)break;const _=v.properties||{},g=this.#E(_,s,a);if(g.length>0){const{label:w,matchedProperty:x}=this.#v(_,g);n.push({type:"feature",label:w,subtype:x,feature:v,layer:m,bounds:this.#A(v,m),coordinates:this.#b(v,m),source:c})}}}const h=this.#n.querySelectorAll("dm-marker[label]");for(const c of h){if(n.length>=5)break;const u=c.label;if(u&&u.toLowerCase().includes(s)){const m=c.center;if(!m)continue;n.push({type:"marker",label:u,subtype:"marker",coordinates:m,bounds:null,marker:c})}}return n}#E(e,n,s){const a=[];for(const[l,h]of Object.entries(e))s&&!s.includes(l)||typeof h=="string"&&h.toLowerCase().includes(n)&&a.push({key:l,value:h});return a}#v(e,n){const s=n[0],a=this.searchLabels,l=this.searchProperties;let h=s.key;if(l&&a){const c=l.indexOf(s.key);c>=0&&a[c]&&(h=a[c])}return{label:s.value,matchedProperty:h}}#A(e,n){const s=e.geometry;if(!s||s.type==="Point")return null;try{let l=null;if(n.eachLayer(h=>{h.feature===e&&(l=h)}),l&&l.getBounds)return l.getBounds()}catch{}return null}#b(e,n){const s=e.geometry;if(!s)return null;const a=s.type,l=s.coordinates;if(a==="Point")return[l[1],l[0]];try{let h=null;if(n.eachLayer(c=>{c.feature===e&&(h=c)}),h){if(h.getBounds){const c=h.getBounds().getCenter();return[c.lat,c.lng]}if(h.getLatLng){const c=h.getLatLng();return[c.lat,c.lng]}}}catch{}return null}#x(e){if(!e)return null;const n=e.match(/POINT\s*\(\s*([\d.-]+)\s+([\d.-]+)\s*\)/i);if(!n)return null;const s=parseFloat(n[1]),a=parseFloat(n[2]);return isNaN(a)||isNaN(s)?null:[a,s]}#B(e){if(!e)return null;const n=e.match(/^(\w+)/i);if(!n)return null;const s=n[1].toUpperCase();try{if(s==="POLYGON"||s==="MULTIPOLYGON")return this.#O(e,s);if(s==="LINESTRING"||s==="MULTILINESTRING")return this.#G(e,s);if(s==="POINT"){const a=this.#x(e);return a?{type:"Point",coordinates:[a[1],a[0]]}:null}}catch{}return null}#O(e,n){const s=[],a=/\(\s*([\d.\s,-]+)\s*\)/g;let l;for(;(l=a.exec(e))!==null;){const h=l[1].split(",").map(c=>{const[u,m]=c.trim().split(/\s+/).map(parseFloat);return[m,u]});h.length>0&&s.push(h)}return s.length===0?null:{type:n==="MULTIPOLYGON"?"MultiPolygon":"Polygon",coordinates:s}}#G(e,n){const s=[];if(n==="LINESTRING"){const a=e.match(/LINESTRING\s*\(\s*([\d.\s,-]+)\s*\)/i);if(a){const l=a[1].split(",").map(h=>{const[c,u]=h.trim().split(/\s+/).map(parseFloat);return[u,c]});s.push(l)}}else{const a=/\(\s*([\d.\s,-]+)\s*\)/g;let l;for(;(l=a.exec(e))!==null;){const h=l[1].split(",").map(c=>{const[u,m]=c.trim().split(/\s+/).map(parseFloat);return[m,u]});h.length>0&&s.push(h)}}return s.length===0?null:{type:n==="MULTILINESTRING"?"MultiLineString":"LineString",coordinates:s}}#F(e){if(e.type==="address"&&e.subtype){const n=e.subtype.toLowerCase();if(n==="provincie")return 5e4;if(n==="gemeente")return 2e4;if(n==="woonplaats")return 15e3;if(n==="wijk"||n==="buurt")return 3e3;if(n==="weg")return 5e3}return this.radius}#q(e=!1){const n=this.#t.querySelector(".search__listbox"),s=this.#t.querySelector(".search__input");if(this.#s.length===0){n.innerHTML=`
			...
			`};let m="";c?(m+='<li class="search__group-label" part="group-label" role="presentation">Op de kaart</li>',m+=l.map(v=>u(v,this.#s.indexOf(v))).join(""),m+='<li class="search__group-label" part="group-label" role="presentation">Adressen</li>',m+=h.map(v=>u(v,this.#s.indexOf(v))).join("")):m+=this.#s.map((v,_)=>u(v,_)).join(""),n.innerHTML=m+a,this.#r>=0?s.setAttribute("aria-activedescendant",`search-option-${this.#r}`):s.removeAttribute("aria-activedescendant")}#Y(e){switch(e.type){case"address":return e.subtype||"Adres";case"feature":return e.subtype||"Object";case"marker":return"Marker";default:return""}}#X(e){switch(e.type){case"address":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-map-pin"/></svg>';case"feature":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-hamburger"/></svg>';case"marker":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-map-pin"/></svg>';default:return""}}#W(){const e=this.#t.querySelector(".search__listbox"),n=this.#t.querySelector(".search__input"),s=this.#t.querySelector(".search__form");e.hidden=!1,n.setAttribute("aria-expanded","true"),s.dataset.open="true"}#k(){const e=this.#t.querySelector(".search__listbox"),n=this.#t.querySelector(".search__input"),s=this.#t.querySelector(".search__form");e.hidden=!0,n.setAttribute("aria-expanded","false"),n.removeAttribute("aria-activedescendant"),this.#r=-1,delete s.dataset.open}#D(e){const n=this.#s.length-1;if(n<0)return;this.#r+=e,this.#r<0?this.#r=n:this.#r>n&&(this.#r=0),this.#q();const s=this.#t.querySelector(`#search-option-${this.#r}`);s&&s.scrollIntoView({block:"nearest"})}#H(e){const n=this.#s[e];if(!n)return;const s=this.#t.querySelector(".search__input"),a=this.#t.querySelector(".search__clear");s.value=n.label,a&&(a.hidden=!n.label),this.#k(),this.#Q(n),this.#J(n),this.dispatchEvent(new CustomEvent("dm-search:select",{bubbles:!0,composed:!0,detail:{type:n.type,label:n.label,coordinates:n.coordinates,bounds:n.bounds,feature:n.feature,marker:n.marker,raw:n.raw}}))}#Q(e){const n=this.#n.map;if(!n)return;this.#T();const s=e.geometry?.type||"";s.includes("Polygon")||s.includes("Line")?this.#R(e):e.type==="address"&&e.coordinates&&this.#U(e.coordinates);let a;if(this.#o?.getBounds)a=this.#o.getBounds();else if(e.bounds)a=e.bounds;else if(e.coordinates){const l=G.latLng(e.coordinates[0],e.coordinates[1]),h=this.#F(e);a=l.toBounds(h)}a&&(n.fitBounds(a,{padding:[40,40]}),this.dispatchEvent(new CustomEvent("dm-search:zoom",{bubbles:!0,composed:!0,detail:{bounds:a,result:e}})))}#R(e){const n=this.#n.map;if(!n||!e.geometry)return;const s=e.geometry,a=s.type==="LineString"||s.type==="MultiLineString",l=this.#V(),h={color:l,weight:a?4:2,opacity:.8,fillColor:l,fillOpacity:a?0:.15};s.type==="Polygon"||s.type==="MultiPolygon"?this.#o=G.polygon(s.coordinates,h):s.type==="LineString"?this.#o=G.polyline(s.coordinates[0],h):s.type==="MultiLineString"&&(this.#o=G.polyline(s.coordinates,h)),this.#o&&this.#o.addTo(n)}#U(e){const n=this.#n?.map;!n||!e||(this.#o=G.circleMarker(e,{radius:9,color:"#fff",weight:2,fillColor:this.#V(),fillOpacity:1,opacity:1}).addTo(n))}#V(){const e=getComputedStyle(this);return e.getPropertyValue("--draad-map-components-search-highlight-color").trim()||e.getPropertyValue("--draad-map-components-color-primary").trim()||"#268641"}#J(e){if(e.type==="marker"&&e.marker)e.marker.active=!0,e.marker.marker?.setZIndexOffset?.(1e3),this.#h=e.marker,e.marker.dispatchEvent(new CustomEvent("dm-marker:click",{bubbles:!0,composed:!0,detail:{marker:e.marker.marker??null,targetId:e.marker.target}}));else if(e.type==="feature"&&e.source){const n=e.source.tagName==="DM-WFS"?"wfs":"geojson";e.source.dispatchEvent(new CustomEvent(`dm-${n}:feature-click`,{bubbles:!0,composed:!0,detail:{feature:e.feature,layer:e.layer,latlng:null}}))}}#T(){this.#o&&this.#n?.map&&(this.#n.map.removeLayer(this.#o),this.#o=null),this.#n?.querySelectorAll("dm-geojson, dm-wfs").forEach(e=>e.clearActiveFeature?.()),this.#h&&(this.#h.active=!1,this.#h.marker?.setZIndexOffset?.(0),this.#h=null)}#j(e){const n=this.#t.querySelector(".search__button");e?(n.classList.add("search__button--loading"),n.setAttribute("aria-busy","true")):(n.classList.remove("search__button--loading"),n.removeAttribute("aria-busy"))}#Z(e){const n=this.#t.querySelector('[role="status"]');n&&(n.textContent="",setTimeout(()=>{n.textContent=e},100))}#z(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}#K(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}disconnectedCallback(){this.#i&&(this.#i.abort(),this.#i=null),this.#a&&(clearTimeout(this.#a),this.#a=null),this.#l&&(clearTimeout(this.#l),this.#l=null),this.#T(),this.#n=null,this.#s=[]}}customElements.define("dm-search",tu);const eu=`/* dm-infowindow - Card panel styling */
:host {
	display: flex;
	flex-direction: column;
	max-block-size: 100%;
...
`;class iu extends HTMLElement{#t;#n=null;#e=null;#i=null;#a=null;#l=null;#r=null;#s=null;#o=null;#h=null;#d=!1;#u;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#u=this.#c()}#c(){const e=new CSSStyleSheet;e.replaceSync(eu),this.#t.adoptedStyleSheets=[e]}static get observedAttributes(){return["for","feature-id","trigger"]}get for(){return this.getAttribute("for")}set for(e){this.setAttribute("for",e)}get featureId(){return this.getAttribute("feature-id")}set featureId(e){this.setAttribute("feature-id",e)}get trigger(){return this.getAttribute("trigger")||"click"}set trigger(e){this.setAttribute("trigger",e)}get isOpen(){return this.#d}connectedCallback(){const e=this.closest("dm-map");e&&(this.#n=e,this.setAttribute("slot","top-right"),this.#_(),this.setAttribute("hidden",""),Ut(this.#t),e.map?this.#f():e.addEventListener("dm-map:ready",()=>{this.#f()},{once:!0}))}async#f(){await this.#u,this.#y(),this.#l=e=>this.#m(e),this.#n.addEventListener("dm-infowindow:open",this.#l)}#m(e){e.target!==this&&this.isOpen&&this.close()}#_(){this.#t.innerHTML=`
		...
		`,this.#t.querySelector(".infowindow__close").addEventListener("click",()=>this.close())}#y(){const e=this.for;e&&(this.#e=document.getElementById(e),this.#e?this.#p():this.#L(e))}#L(e){this.#s&&this.#s.disconnect(),this.#s=new MutationObserver((n,s)=>{const a=document.getElementById(e);a&&(this.#e=a,this.#p(),s.disconnect(),this.#s=null)}),this.#s.observe(this.#n,{childList:!0,subtree:!0})}#p(){const e=this.trigger,n=this.#e.tagName.toLowerCase();n==="dm-marker"?this.#P(e):(n==="dm-geojson"||n==="dm-wfs")&&this.#S(e,n)}#w(){if(this.#e&&(this.#i&&(this.#e.removeEventListener("dm-marker:click",this.#i),this.#i=null),this.#a)){const e=this.#e.tagName.toLowerCase().replace("dm-","");this.#e.removeEventListener(`dm-${e}:feature-click`,this.#a),this.#a=null}}#P(e){e==="click"&&(this.#i=n=>this.#g(n),this.#e.addEventListener("dm-marker:click",this.#i))}#S(e,n){const a=`dm-${n.replace("dm-","")}:feature-click`;e==="click"&&(this.#a=l=>this.#N(l),this.#e.addEventListener(a,this.#a))}#g(e){if(this.#h=null,this.isOpen)this.close();else{const n=this.#E();this.#F(n),this.open()}}#N(e){const{feature:n}=e.detail;if(!this.#C(n))return;this.#h=n;const s=this.#v(n);this.#F(s),this.#B(this,n),this.open()}#C(e){const n=this.featureId;return!n||n==="*"||e.id!==void 0&&String(e.id)===n||e.properties?.id!==void 0&&String(e.properties.id)===n}#E(){const e=document.createElement("div"),n=this.querySelector("template");if(n)e.appendChild(n.content.cloneNode(!0));else for(const s of this.childNodes)s.nodeType===Node.ELEMENT_NODE&&s.hasAttribute("slot")||e.appendChild(s.cloneNode(!0));return this.#b(e),e}#v(e){const n=document.createElement("div"),s=this.querySelector("template");if(s){const a=s.content.cloneNode(!0);n.appendChild(a),this.#B(n,e),this.#O(n)}else if(this.#e&&this.#A()){const a=this.#x(e);a&&n.appendChild(a)}else for(const a of this.childNodes)a.nodeType===Node.ELEMENT_NODE&&a.hasAttribute("slot")||n.appendChild(a.cloneNode(!0));return this.#b(n),n}#A(){return this.#e?.getAttribute("infowindow-properties")}#b(e){const n=a=>{const l=document.createElementNS("http://www.w3.org/2000/svg","svg");l.setAttribute("aria-hidden","true");const h=document.createElementNS("http://www.w3.org/2000/svg","use");return h.setAttribute("href",`#${a}`),l.appendChild(h),l},s=a=>{const l=a.getAttribute("href");if(!l)return!1;try{return new URL(l,window.location.href).origin!==window.location.origin}catch{return!1}};e.querySelectorAll(".address, address").forEach(a=>{a.setAttribute("part","address"),a.prepend(n("icon-map-pin"))}),e.querySelectorAll(".action, a.button, button.action").forEach(a=>{a.setAttribute("part","action"),s(a)&&a.prepend(n("icon-external-link"))})}#x(e){const n=this.#e.getAttribute("infowindow-properties"),s=this.#e.getAttribute("infowindow-labels");if(!n)return null;const a=n.split(",").map(c=>c.trim()),l=s?s.split(",").map(c=>c.trim()):a,h=document.createElement("dl");h.className="dm-infowindow-data",h.setAttribute("part","data-table");for(let c=0;c<a.length;c++){const u=e.properties?.[a[c]];if(u==null||u==="")continue;const m=document.createElement("dt");m.textContent=l[c]||a[c],h.appendChild(m);const v=document.createElement("dd");v.innerHTML=u!==null&&typeof u=="object"?JSON.stringify(u):String(u),h.appendChild(v)}return h.children.length>0?h:null}#B(e,n){const s=new Set(["https:","http:","mailto:","tel:"]),a=(l,h)=>{l.style.display=h?"":"none"};e.querySelectorAll("[data-field]").forEach(l=>{const h=this.#G(n,l.getAttribute("data-field")),c=h!=null?String(h):"";l.textContent=c,a(l,c!=="")}),e.querySelectorAll("[data-href]").forEach(l=>{const h=this.#G(n,l.getAttribute("data-href"));let c=!1;if(h!=null)try{const u=new URL(String(h),window.location.origin);s.has(u.protocol)?(l.setAttribute("href",String(h)),c=!0):l.removeAttribute("href")}catch{l.removeAttribute("href")}else l.removeAttribute("href");a(l,c)}),e.querySelectorAll("[data-src]").forEach(l=>{const h=this.#G(n,l.getAttribute("data-src"));let c=!1;if(h!=null)try{const u=new URL(String(h),window.location.origin);u.protocol==="https:"||u.protocol==="http:"?(l.setAttribute("src",String(h)),c=!0):l.removeAttribute("src")}catch{l.removeAttribute("src")}else l.removeAttribute("src");a(l,c)})}#O(e){e.querySelectorAll("dl.dm-infowindow-data").forEach(n=>{const s=Array.from(n.children);let a=!1;s.forEach((l,h)=>{if(l.tagName!=="DD")return;if(l.style.display==="none"||l.textContent===""){l.style.display="none";const u=s[h-1];u&&u.tagName==="DT"&&(u.style.display="none")}else a=!0}),a||(n.style.display="none")})}#G(e,n){if(!n.startsWith("properties.")&&n!=="id"&&n!=="properties")return;const s=new Set(["__proto__","constructor","prototype"]),a=n.split(".");let l=e;for(const h of a){if(l==null||s.has(h)||!Object.prototype.hasOwnProperty.call(l,h))return;l=l[h]}return l}#F(e){const n=this.#t.querySelector(".infowindow__content");for(n.innerHTML="";e.firstChild;)n.appendChild(e.firstChild);n.querySelectorAll("a").forEach(s=>s.setAttribute("part","link"))}open(){this.#t.querySelector(".infowindow").removeAttribute("hidden"),this.removeAttribute("hidden"),this.#d=!0,this.#q(),this.dispatchEvent(new CustomEvent("dm-infowindow:open",{bubbles:!0,composed:!0,detail:{target:this.#e,feature:this.#h}}))}close(){this.#t.querySelector(".infowindow").setAttribute("hidden",""),this.setAttribute("hidden",""),this.#d=!1,this.#r&&(document.removeEventListener("keydown",this.#r),this.#r=null),this.#e?.tagName.toLowerCase()==="dm-marker"&&(this.#e.active=!1),this.#o&&typeof this.#o.focus=="function"&&this.#o.focus(),this.#o=null,this.dispatchEvent(new CustomEvent("dm-infowindow:close",{bubbles:!0,composed:!0,detail:{target:this.#e}}))}toggle(){this.isOpen?this.close():this.open()}#q(){const e=this.#t.querySelector(".infowindow");this.#o=document.activeElement,e.setAttribute("tabindex","-1"),e.focus(),this.#r=n=>{n.key==="Escape"&&this.isOpen&&this.close()},document.addEventListener("keydown",this.#r)}attributeChangedCallback(e,n,s){if(!(n===s||!this.#n))switch(e){case"for":this.#w(),this.#e=null,this.#y();break;case"trigger":this.#w(),this.#e&&this.#p();break}}disconnectedCallback(){this.#w(),this.#l&&(this.#n?.removeEventListener("dm-infowindow:open",this.#l),this.#l=null),this.#r&&(document.removeEventListener("keydown",this.#r),this.#r=null),this.#s&&(this.#s.disconnect(),this.#s=null),this.#n=null,this.#e=null,this.#o=null,this.#h=null}}customElements.define("dm-infowindow",iu);const nu=`.icon {
	display: block;
	inline-size: 1em;
	block-size: 1em;
	fill: currentColor;
...
`,au=new Set(["http:","https:","mailto:","tel:"]),ou=new Set(["__proto__","constructor","prototype"]);class lu extends HTMLElement{#t;#n=[];#e=new Map;#i=null;#a=null;#l=!1;#r;#s=null;#o=null;#h=new Map;#d=new Map;#u=null;#c=1;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#r=this.#f()}#f(){const e=new CSSStyleSheet;e.replaceSync(ms),this.#t.adoptedStyleSheets=[e],this.#o=new CSSStyleSheet,this.#o.replaceSync(su)}static get observedAttributes(){return["for","label","columns","per-page","prev-label","next-label","page-label"]}attributeChangedCallback(e){this.#l&&(e==="per-page"&&(this.#c=1),this.#g())}get for(){return this.getAttribute("for")}get label(){return this.getAttribute("label")||"Lijst bekijken"}get shortLabel(){return this.getAttribute("short-label")||"Lijst"}get columns(){const e=parseInt(this.getAttribute("columns"),10);return e>=1&&e<=4?e:3}get perPage(){const e=parseInt(this.getAttribute("per-page"),10);return e>=1&&e<=200?e:12}get prevLabel(){return this.getAttribute("prev-label")||"Vorige"}get nextLabel(){return this.getAttribute("next-label")||"Volgende"}get pageLabelTemplate(){return this.getAttribute("page-label")||"Pagina {current} van {total}"}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#m(e):(this.#u=()=>this.#m(e),e.addEventListener("dm-map:ready",this.#u,{once:!0})))}async#m(e){this.#s=e,await this.#r,this.#_(),Ut(this.#t),this.#y(),this.#L()}#_(){const e=document.createElement("div");e.className="list",e.setAttribute("part","toggle-wrapper");const n=document.createElement("button");n.className="list__toggle",n.type="button",n.setAttribute("part","toggle"),n.setAttribute("aria-pressed","false"),n.setAttribute("aria-label",this.label),n.innerHTML='<svg class="list__icon" part="toggle-icon" aria-hidden="true"><use href="#icon-list"/></svg>';const s=document.createElement("span");s.className="list__text list__text--full",s.setAttribute("part","toggle-text-full"),s.textContent=this.label,n.appendChild(s);const a=document.createElement("span");a.className="list__text list__text--short",a.setAttribute("part","toggle-text-short"),a.textContent=this.shortLabel,n.appendChild(a),n.addEventListener("click",()=>this.#p()),e.appendChild(n),this.#t.appendChild(e)}#y(){const e=this.for;if(!e)return;const n=e.split(",").map(s=>s.trim()).filter(Boolean);this.#n=[],n.forEach(s=>{const a=this.#s.querySelector(`#${CSS.escape(s)}`);a&&this.#n.push(a)})}#L(){this.#n.forEach(e=>{const n=h=>{this.#e.set(e.id,h.detail.filters),this.#c=1,this.#l&&this.#g()};this.#h.set(e,n),e.addEventListener("dm-filter:apply",n);const s=e.tagName.toLowerCase(),a=s==="dm-geojson"?"dm-geojson:ready":s==="dm-wfs"?"dm-wfs:ready":"dm-layer:ready",l=()=>{this.#c=1,this.#l&&this.#g()};this.#d.set(e,{event:a,handler:l}),e.addEventListener(a,l)})}#p(){this.#l?this.#P():this.#w()}#w(){this.#l=!0;const e=this.#t.querySelector(".list__toggle");e.setAttribute("aria-pressed","true");const n=e.querySelector(".list__icon");e.querySelector(".list__text--full").textContent="Kaart bekijken",e.querySelector(".list__text--short").textContent="Kaart",n.innerHTML='<use href="#icon-map"/>',this.#i||(this.#i=document.createElement("div"),this.#i.setAttribute("slot","list"),this.#a=this.#i.attachShadow({mode:"open"}),this.#o&&(this.#a.adoptedStyleSheets=[this.#o]),this.#s.appendChild(this.#i)),this.#c=1,this.#g(),this.dispatchEvent(new CustomEvent("dm-list:open",{bubbles:!0,composed:!0,detail:{source:this}})),requestAnimationFrame(()=>{const s=this.#a?.querySelector(".list__card");s&&s.focus()})}#P(){this.#l=!1;const e=this.#t.querySelector(".list__toggle");e.setAttribute("aria-pressed","false");const n=e.querySelector(".list__icon");e.querySelector(".list__text--full").textContent=this.label,e.querySelector(".list__text--short").textContent=this.shortLabel,n.innerHTML='<use href="#icon-list"/>',e.focus(),this.dispatchEvent(new CustomEvent("dm-list:close",{bubbles:!0,composed:!0,detail:{source:this}}))}#S(){const e=[];return this.#n.forEach(n=>{const s=n.data;if(!s?.features)return;let a=s.features;const l=this.#e.get(n.id);l&&Object.keys(l).length>0&&(a=Pe(a,l)),a.forEach(h=>{e.push({feature:h,source:n})})}),e}#g(){if(!this.#a)return;const e=this.querySelector("template"),n=this.#S(),s=n.length,a=this.perPage,l=Math.max(1,Math.ceil(s/a));this.#c>l&&(this.#c=l),this.#c<1&&(this.#c=1);const h=(this.#c-1)*a,c=Math.min(h+a,s),u=n.slice(h,c);if(n.length===0){this.#a.innerHTML=`
				<div class="list__panel" part="panel">
					<p class="list__empty" part="empty">Geen resultaten gevonden.</p>
				</div>
			`,Ut(this.#a);return}const m=document.createElement("div");m.className="list__panel",m.setAttribute("part","panel"),m.style.setProperty("--draad-map-components-list-columns",this.columns);const v=document.createElement("p");v.className="list__count",v.setAttribute("part","count"),v.setAttribute("role","status"),v.setAttribute("aria-live","polite"),v.textContent=s<=a?`${s} ${s!==1?"resultaten":"resultaat"}`:`${h+1}–${c} van ${s} resultaten`,m.appendChild(v);const _=document.createElement("div");_.className="list__grid",_.setAttribute("part","grid");const g=e?.content.querySelector("a[href], [data-href]")??null;if(u.forEach(({feature:w,source:x},S)=>{const P=h+S,C=document.createElement("article");if(C.className="list__card",C.setAttribute("part","card"),C.setAttribute("tabindex","0"),g||C.setAttribute("role","button"),e){const E=e.content.cloneNode(!0);this.#C(E,w),C.appendChild(E)}else{const E=w.properties?.name||w.properties?.naam||w.id||`Feature ${P+1}`,z=document.createElement("h3");z.textContent=String(E),C.appendChild(z)}this.#N(C),C.addEventListener("click",E=>{E.target.closest("a[href]")||(this.dispatchEvent(new CustomEvent("dm-list:item-click",{bubbles:!0,composed:!0,detail:{feature:w,index:P,source:x}})),this.#P(),this.#v(w,x))}),C.addEventListener("keydown",E=>{E.key==="Enter"&&!E.target.closest("a[href]")&&C.click()}),_.appendChild(C)}),m.appendChild(_),l>1){const w=document.createElement("nav");w.className="list__pagination",w.setAttribute("part","pagination"),w.setAttribute("aria-label","Paginering");const x=document.createElement("button");x.type="button",x.className="list__pagination-arrow",x.setAttribute("part","pagination-prev"),x.setAttribute("aria-label",this.prevLabel),x.dataset.dir="prev",this.#c===1&&(x.disabled=!0),x.innerHTML='<svg class="list__pagination-icon" aria-hidden="true" focusable="false"><use href="#icon-chevron-left"/></svg>';const S=document.createElement("div");S.className="list__pagination-pages",S.setAttribute("role","status"),S.setAttribute("aria-live","polite"),S.setAttribute("aria-label",this.pageLabelTemplate.replace("{current}",this.#c).replace("{total}",l)),this.#b(this.#c,l).forEach(C=>{if(C==="..."){const E=document.createElement("span");E.className="list__pagination-ellipsis",E.setAttribute("aria-hidden","true"),E.textContent="…",S.appendChild(E)}else{const E=document.createElement("button");E.type="button",E.className="list__pagination-page",E.dataset.page=C,E.textContent=C,C===this.#c&&(E.classList.add("list__pagination-page--active"),E.setAttribute("aria-current","page")),S.appendChild(E)}});const P=document.createElement("button");P.type="button",P.className="list__pagination-arrow",P.setAttribute("part","pagination-next"),P.setAttribute("aria-label",this.nextLabel),P.dataset.dir="next",this.#c===l&&(P.disabled=!0),P.innerHTML='<svg class="list__pagination-icon" aria-hidden="true" focusable="false"><use href="#icon-chevron-right"/></svg>',w.append(x,S,P),w.addEventListener("click",C=>{const E=C.target.closest("button[data-page]");if(E&&!E.disabled){this.#x(parseInt(E.dataset.page,10));return}const z=C.target.closest("button[data-dir]");!z||z.disabled||this.#x(z.dataset.dir==="next"?this.#c+1:this.#c-1)}),m.appendChild(w)}this.#a.innerHTML="",Ut(this.#a),this.#a.appendChild(m)}#N(e){const n=a=>{const l=document.createElementNS("http://www.w3.org/2000/svg","svg");l.setAttribute("aria-hidden","true");const h=document.createElementNS("http://www.w3.org/2000/svg","use");return h.setAttribute("href",`#${a}`),l.appendChild(h),l},s=a=>{const l=a.getAttribute("href");if(!l)return!1;try{return new URL(l,window.location.href).origin!==window.location.origin}catch{return!1}};e.querySelectorAll(".address, address").forEach(a=>a.prepend(n("icon-map-pin"))),e.querySelectorAll(".action, a.button").forEach(a=>{s(a)&&a.prepend(n("icon-external-link"))})}#C(e,n){const s=(a,l)=>{a.style.display=l?"":"none"};e.querySelectorAll("[data-field]").forEach(a=>{const l=this.#E(n,a.getAttribute("data-field")),h=l!=null?String(l):"";a.textContent=h,s(a,h!=="")}),e.querySelectorAll("[data-href]").forEach(a=>{const l=a.getAttribute("data-href"),h=this.#E(n,l);if(h!=null){const c=String(h);try{const u=new URL(c,window.location.origin);au.has(u.protocol)?a.setAttribute("href",c):a.removeAttribute("href")}catch{a.removeAttribute("href")}}else a.removeAttribute("href")}),e.querySelectorAll("[data-src]").forEach(a=>{const l=a.getAttribute("data-src"),h=this.#E(n,l);let c=!1;if(h!=null){const u=String(h);try{const m=new URL(u,window.location.origin);m.protocol==="https:"||m.protocol==="http:"?(a.setAttribute("src",u),c=!0):a.removeAttribute("src")}catch{a.removeAttribute("src")}}else a.removeAttribute("src");s(a,c)}),e.querySelectorAll("[data-chips]").forEach(a=>{const l=a.getAttribute("data-chips"),h=this.#E(n,l);if(h!=null){const c=String(h).split(",").map(u=>u.trim()).filter(Boolean);a.innerHTML="",c.forEach(u=>{const m=document.createElement("span");m.className="chip",m.textContent=u,a.appendChild(m)})}}),e.querySelectorAll("dl.dm-infowindow-data").forEach(a=>{let l=!1;a.querySelectorAll("dd").forEach(h=>{const c=h.style.display==="none"||h.textContent.trim()==="",u=h.previousElementSibling;u&&u.tagName==="DT"&&s(u,!c),c||(l=!0)}),l||s(a,!1)}),e.querySelectorAll(".chips").forEach(a=>{[...a.children].some(h=>h.style.display!=="none"&&h.textContent.trim()!=="")||s(a,!1)})}#E(e,n){if(!n.startsWith("properties.")&&n!=="id"&&n!=="properties")return;const s=n.split(".");let a=e;for(const l of s){if(a==null||ou.has(l)||!Object.prototype.hasOwnProperty.call(a,l))return;a=a[l]}return a}async#v(e,n){if(!this.#s?.map)return;const s=e.geometry;if(!s)return;const a=this.#s.map,h=kn(e)?Me(n?.getAttribute("crs"))||"EPSG:28992":null,c=(u,m)=>{if(h){const[v,_]=Gi([u,m],h);return[_,v]}return[m,u]};if(s.type==="Point"){const[u,m]=s.coordinates;a.setView(c(u,m),Math.max(a.getZoom(),16))}else try{const u=(await Promise.resolve().then(()=>Ia)).default,m=h?{coordsToLatLng:_=>u.latLng(c(_[0],_[1]))}:{},v=u.geoJSON(e,m);a.fitBounds(v.getBounds(),{padding:[50,50]})}catch{const u=this.#A(s);u&&a.setView(c(u[0],u[1]),Math.max(a.getZoom(),16))}}#A(e){let n=e.coordinates;for(;Array.isArray(n)&&Array.isArray(n[0]);)n=n[0];return Array.isArray(n)&&n.length>=2?n:null}#b(e,n){return n<=5?Array.from({length:n},(s,a)=>a+1):e<=3?[1,2,3,"...",n]:e>=n-2?[1,"...",n-3,n-2,n-1,n]:[1,"...",e-1,e,e+1,"...",n]}#x(e){const n=this.perPage,s=this.#S().length,a=Math.max(1,Math.ceil(s/n));this.#c=Math.max(1,Math.min(e,a)),this.#g(),this.#i?.scrollIntoView({block:"start",behavior:"smooth"}),requestAnimationFrame(()=>{const l=this.#a?.querySelector(".list__card");l&&l.focus()})}disconnectedCallback(){this.#u&&(this.closest("dm-map")?.removeEventListener("dm-map:ready",this.#u),this.#u=null),this.#h.forEach((e,n)=>{n.removeEventListener("dm-filter:apply",e)}),this.#h.clear(),this.#d.forEach(({event:e,handler:n},s)=>{s.removeEventListener(e,n)}),this.#d.clear(),this.#i?.parentNode&&this.#i.parentNode.removeChild(this.#i),this.#i=null,this.#a=null,this.#n=[],this.#e.clear(),this.#s=null}}customElements.define("dm-list",lu);const hu=`.icon {
	display: block;
	inline-size: 1em;
	block-size: 1em;
	fill: currentColor;
...
`;class cu extends HTMLElement{#t;#n=null;#e=[];#i={};#a={};#l=new Map;#r=new WeakMap;#s=new Map;#o=new Map;#h;#d=!1;#u=!1;#c=null;#f=null;#m=null;#_=!1;#y=null;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#h=this.#L()}#L(){const e=new CSSStyleSheet;e.replaceSync(hu),this.#t.adoptedStyleSheets=[e]}get#p(){const e=this.getAttribute("variant");return e==="dropdown"||e==="sidebar"?e:"dialog"}get#w(){return(this.getAttribute("submit")||"auto")==="auto"}get#P(){return this.hasAttribute("collapsible")}get#S(){return this.getAttribute("bool-label")||"Ja"}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#g(e):e.addEventListener("dm-map:ready",()=>{this.#g(e)},{once:!0}))}async#g(e){if(this.#n=e,await this.#h,this.#N(),this.#C(),this.#G(),Ut(this.#t),this.#V(),this.#k(),this.#T(),this.hasAttribute("open")&&this.#p==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__toggle");s?.classList.add("filter__panel--open"),a?.setAttribute("aria-expanded","true"),this.#u=!0}this.#et()&&(this.#d=!0,this.#a=JSON.parse(JSON.stringify(this.#i))),this.#y=()=>this.#et(!0),window.addEventListener("popstate",this.#y)}#N(){const e=["dm-geojson[filter-property], dm-geojson[filter-properties]","dm-wfs[filter-property], dm-wfs[filter-properties]","dm-layer[filter-property], dm-layer[filter-properties]"];this.#e=[],e.forEach(n=>{this.#n.querySelectorAll(n).forEach(s=>{const a=s.getAttribute("filter-properties"),l=s.getAttribute("filter-labels"),h=s.getAttribute("filter-property"),c=s.getAttribute("filter-label"),u=s.getAttribute("filter-types"),m=s.getAttribute("filter-collapsed"),v=s.getAttribute("filter-bool-labels");let _=[],g=[],w=[],x=[],S=[];if(a){for(_=a.split(",").map(E=>E.trim()).filter(Boolean),l&&(g=l.split(",").map(E=>E.trim()).filter(Boolean)),v&&(S=v.split(",").map(E=>E.trim())),u&&(w=u.split(",").map(E=>E.trim()).filter(Boolean)),m&&(x=m.split(",").map(E=>E.trim()).filter(Boolean));g.length<_.length;)g.push(_[g.length]);for(;w.length<_.length;)w.push("auto");for(;x.length<_.length;)x.push("false");for(;S.length<_.length;)S.push("")}else h&&(_=[h],g=[c||h],w=[u||"auto"],x=[m||"false"],S=[v||""]);if(_.length===0)return;const P=s.id||`filter-source-${this.#e.length}`,C=s.getAttribute("filter-key");if(_.forEach((E,z)=>{this.#e.push({element:s,label:g[z],property:E,groupKey:C||E,type:s.tagName.toLowerCase(),id:P,filterType:w[z]||"auto",collapsed:x[z]==="true",boolLabel:S[z]||""})}),!this.#r.has(s))if(this.#r.set(s,!0),s.data)_.forEach(E=>{this.#v(s,E)});else{const E=s.tagName==="DM-GEOJSON"?"dm-geojson:ready":s.tagName==="DM-WFS"?"dm-wfs:ready":"dm-layer:ready";let z=!1;s.addEventListener(E,()=>{z||(z=!0,_.forEach(R=>{this.#v(s,R)}),this.#C(),this.#k(),this.#T())},{once:!0}),s.data&&!z&&(z=!0,_.forEach(R=>{this.#v(s,R)}))}})})}#C(){this.#o=new Map,this.#e.forEach(e=>{this.#o.has(e.groupKey)||this.#o.set(e.groupKey,{label:e.label,filterType:e.filterType,collapsed:e.collapsed,boolLabel:e.boolLabel,sources:[]});const n=this.#o.get(e.groupKey);n.sources.find(a=>a.element===e.element&&a.property===e.property)||n.sources.push({element:e.element,id:e.id,property:e.property})})}#E(e,n){const s=new Set;if(n.sources.forEach(({id:l,property:h})=>{const c=`${l}:${h}`,u=this.#l.get(c);u&&u.values.forEach(m=>s.add(m))}),s.size===0)return null;const a=Array.from(s).sort((l,h)=>typeof l=="number"&&typeof h=="number"?l-h:String(l).localeCompare(String(h)));return{values:a,valueType:this.#x(a)}}#v(e,n){const s=this.#A(e,n),a=this.#x(s),l=this.#e.find(c=>c.element===e&&c.property===n);if(!l)return;const h=`${l.id}:${n}`;this.#l.set(h,{values:s,valueType:a}),this.#d&&Object.keys(this.#i).length>0&&this.#z(),this.#H()&&(this.#k(),this.#T())}#A(e,n){if(!e.data?.features)return[];const s=new Set;return e.data.features.forEach(a=>{const l=a.properties?.[n];if(l!=null){const h=String(l);h.includes(",")?h.split(",").forEach(c=>{const u=c.trim();u&&s.add(u)}):s.add(l)}}),Array.from(s).sort((a,l)=>typeof a=="number"&&typeof l=="number"?a-l:String(a).localeCompare(String(l)))}#b(e,n){const s=new Map;return n.sources.forEach(({element:a,property:l})=>{a.data?.features&&a.data.features.forEach(h=>{const c=h.properties?.[l];if(c==null)return;const u=String(c);(u.includes(",")?u.split(",").map(v=>v.trim()).filter(Boolean):[u]).forEach(v=>{s.set(v,(s.get(v)||0)+1)})})}),s}#x(e){if(e.length===0)return{isNumeric:!1};if(e.length===1)return{isNumeric:!1,single:!0};if(e.every(s=>typeof s=="number"?!0:typeof s=="string"?!isNaN(Number(s))&&s.trim()!=="":!1)){const s=e.map(a=>Number(a));return{isNumeric:!0,min:Math.min(...s),max:Math.max(...s)}}return{isNumeric:!1}}#B(e,n){const s=this.#o.get(e)?.filterType;return s&&s!=="auto"?s==="range"&&!n.valueType.isNumeric?this.#O(n):s:this.#O(n)}#O({values:e,valueType:n}){return n.isNumeric&&!n.single?"range":e.length<=20?"checkbox":"dropdown"}#G(){const e=this.#p;e==="dropdown"?this.#X():e==="sidebar"?this.#W():this.#Y()}#F(){return`
			<button
				part="toggle"
		...
		`}#q(){const e=this.#w?" filter--auto-submit":"",n=this.getAttribute("label")||"Filters";return`
		...
		`}#Y(){const e=this.getAttribute("label")||"Filters",n=this.#w?" filter--auto-submit":"";this.#t.innerHTML=`
			<div part="root" class="filter filter--dialog${n}">
				${this.#F()}

				<dialog id="filter-panel" part="dialog" class="filter__dialog" role="dialog" aria-modal="true" aria-label="${this.#M(e)}">
					${this.#q()}
				</dialog>
		...
		`}#X(){const e=this.getAttribute("label")||"Filters",n=this.#w?" filter--auto-submit":"";this.#t.innerHTML=`
			<div part="root" class="filter filter--dropdown${n}">
				...
				<div id="filter-panel" part="dropdown" class="filter__panel filter__panel--dropdown" role="region" aria-label="${this.#M(e)}">
					${this.#q()}
				</div>

				<div role="status" aria-live="polite" aria-atomic="true" class="visually-hidden"></div>
			</div>
		`}#W(){const e=this.getAttribute("label")||"Filters",n=this.#w?" filter--auto-submit":"";this.#t.innerHTML=`
			<div part="root" class="filter filter--sidebar${n}">
				${this.#F()}

				<div class="filter__overlay" part="overlay" aria-hidden="true"></div>

				<div id="filter-panel" part="sidebar" class="filter__panel filter__panel--sidebar" role="region" aria-label="${this.#M(e)}">
					${this.#q()}
				</div>

				<div role="status" aria-live="polite" aria-atomic="true" class="visually-hidden"></div>
			</div>
		`}#k(){if(!this.#_){this.#_=!0;try{const e=this.#t.querySelector(".filter__content");if(!e)return;if(this.#o.size===0){e.innerHTML='<p class="filter__empty">Geen filterbare gegevensbronnen gevonden.</p>';return}const n=[];this.#o.forEach((s,a)=>{let l=this.#E(a,s);if(l||(s.sources.forEach(({element:E,property:z})=>{E.data&&this.#v(E,z)}),l=this.#E(a,s)),!l||l.values.length===0){n.push(`
						<div class="filter__section" part="section" data-property="${this.#M(a)}">
							<h3 class="filter__section-title" part="section-title">${this.#I(s.label)}</h3>
							<p class="filter__loading">Loading data...</p>
						</div>
					`);return}const{values:h,valueType:c}=l,u=this.#B(a,l),m=s.collapsed,v=m?"false":"true",_=m?" hidden":"";if(u==="bool"){const E=this.#i[a]?.[0]==="1"?" checked":"",z=s.boolLabel||this.#S,R=this.#I(z),F=`
						<label class="filter__checkbox-label">
							<input
								type="checkbox"
								class="filter__checkbox"
								value="1"
								aria-label="${this.#I(`${s.label}: ${z}`)}"
								${E}
							>
							<span class="filter__checkbox-text">${R}</span>
						</label>`;n.push(`
						<div class="filter__section filter__section--bool" part="section" data-property="${this.#M(a)}">
							${this.#D(s.label,a,v,_,F)}
						</div>
					`);return}if(u==="single"||c.single){n.push(`
						<div class="filter__section" part="section" data-property="${this.#M(a)}">
							${this.#D(s.label,a,v,_,`
								<p class="filter__single">Value: ${this.#I(String(h[0]))}</p>
							`)}
						</div>
						...
						<div class="filter__range">
							<label class="filter__range-label">
								...
								<input
									...
									aria-label="${this.#M(s.label)} minimum value"
								>
							</label>
							<label class="filter__range-label">
								...
								<input
									...
									value="${z}"
									step="${this.#it(c.min,c.max)}"
									aria-label="${this.#M(s.label)} maximum value"
								>
							</label>
						</div>`;n.push(`
						<div class="filter__section" part="section" data-property="${this.#M(a)}">
					...
					`);return}if(u==="checkbox"){const E=s.sources.reduce((F,{element:J})=>F||this.#ht(J),null),z=this.#b(a,s),R=h.map(F=>{const J=this.#st(a,F)?"checked":"",ot=this.#M(String(F)),et=this.#I(String(F)),St=z.get(String(F))||0,Q=St>0?` (${St})`:"",ht=E?.[F]?this.#ct(E[F]):this.#$();return`
							<label class="filter__checkbox-label">
								<input
									type="checkbox"
									class="filter__checkbox"
					...
					`);return}const g=this.#i[a]||[],w=g.map(E=>{const z=this.#M(String(E)),R=this.#I(String(E));return`
					...
					`}).join(""),x=h.filter(E=>!g.includes(String(E))).map(E=>{const z=this.#M(String(E)),R=this.#I(String(E));return`
							<div class="filter__option" data-value="${z}" role="option" tabindex="0">
								${R}
							</div>
						`}).join(""),S=`filter-select-${a}`,P=g.length===0?"Selecteer waarden...":"",C=`
					<div class="filter__custom-select" part="select" id="${S}" role="combobox" aria-expanded="false" aria-haspopup="listbox" aria-label="${this.#M(s.label)}" tabindex="0">
				...
				`)}),e.innerHTML=n.join("")}finally{this.#_=!1}}}#D(e,n,s,a,l){if(this.#P){const h=`filter-section-${this.#M(n)}`;return`
			...
			${l}`}#H(){return this.#p==="dialog"?this.#t.querySelector("dialog")?.open??!1:this.#u}#Q(){const e=this.#p,n=this.#t.querySelector(".filter__toggle");if(this.#k(),this.#T(),e==="dialog")this.#t.querySelector("dialog").showModal(),n.setAttribute("aria-expanded","true");else if(e==="dropdown"){const s=this.#t.querySelector(".filter__panel--dropdown"),a=()=>{const l=n.getBoundingClientRect(),h=this.closest("dm-map"),c=h?h.getBoundingClientRect():null;s.style.top=`${l.bottom+4}px`,c?(s.style.left=`${c.left}px`,s.style.right=`${window.innerWidth-c.right}px`,s.style.inlineSize="auto"):(s.style.left="",s.style.right=`${window.innerWidth-l.right}px`,s.style.inlineSize="")};a(),s.classList.add("filter__panel--open"),n.setAttribute("aria-expanded","true"),this.#u=!0,this.#f=a,window.addEventListener("scroll",this.#f,{passive:!0,capture:!0}),this.#c=l=>{l.composedPath().some(h=>h===s||h===n)||this.#R()},requestAnimationFrame(()=>{document.addEventListener("click",this.#c,!0)}),this.#t.addEventListener("keydown",this.#U)}else if(e==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__overlay");s.classList.add("filter__panel--open"),a.classList.add("filter__overlay--visible"),n.setAttribute("aria-expanded","true"),this.#u=!0,this.#t.addEventListener("keydown",this.#U)}}#R(){const e=this.#p,n=this.#t.querySelector(".filter__toggle");if(e==="dialog")this.#t.querySelector("dialog").close();else if(e==="dropdown")this.#t.querySelector(".filter__panel--dropdown")?.classList.remove("filter__panel--open"),this.#u=!1,this.#f&&(window.removeEventListener("scroll",this.#f,{capture:!0}),this.#f=null),this.#c&&(document.removeEventListener("click",this.#c,!0),this.#c=null),this.#t.removeEventListener("keydown",this.#U);else if(e==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__overlay");s?.classList.remove("filter__panel--open"),a?.classList.remove("filter__overlay--visible"),this.#u=!1,this.#t.removeEventListener("keydown",this.#U)}n.setAttribute("aria-expanded","false"),n.focus()}#U=e=>{e.key==="Escape"&&this.#R()};#V(){this.#t.querySelector(".filter__toggle").addEventListener("click",()=>{this.#H()?this.#R():this.#Q()});const e=this.#t.querySelector('slot[name="header"]');if(e){const n=()=>{const s=e.assignedNodes().length>0;e.closest(".filter__header-text")?.classList.toggle("filter__header-text--empty",!s)};e.addEventListener("slotchange",n),n()}if(this.#t.querySelector(".filter__apply").addEventListener("click",()=>{this.#j(),this.#z(),this.#p==="dialog"&&this.#R()}),this.#t.querySelector(".filter__reset").addEventListener("click",()=>this.#K()),this.#t.querySelector(".filter__close").addEventListener("click",()=>this.#R()),this.#p==="dialog"){const n=this.#t.querySelector("dialog");n.addEventListener("close",()=>{const s=this.#t.querySelector(".filter__toggle");s.setAttribute("aria-expanded","false"),s.focus()}),n.addEventListener("click",s=>{s.target===n&&this.#R()})}this.#p==="sidebar"&&this.#t.querySelector(".filter__overlay")?.addEventListener("click",()=>this.#R())}#J(e){const n=e.querySelector(".filter__custom-select-display"),s=e.querySelector(".filter__custom-select-dropdown");if(!n||!s)return;const a=n.getBoundingClientRect(),l=4,h=240,c=window.innerHeight,u=c-a.bottom-l,m=a.top-l,v=u<160&&m>u,_=v?m:u;s.style.left=`${a.left}px`,s.style.width=`${a.width}px`,s.style.maxBlockSize=`${Math.max(120,Math.min(h,_))}px`,v?(s.style.top="auto",s.style.bottom=`${c-a.top+l}px`):(s.style.bottom="auto",s.style.top=`${a.bottom+l}px`)}#T(){const e=this.#t.querySelector(".filter__content");if(!e)return;this.#m?.abort(),this.#m=new AbortController;const{signal:n}=this.#m,s=this.#w;e.querySelectorAll(".filter__section-toggle").forEach(a=>{a.addEventListener("click",()=>{const l=a.getAttribute("aria-expanded")==="true",h=a.getAttribute("aria-controls"),c=this.#t.getElementById(h);c&&(a.setAttribute("aria-expanded",l?"false":"true"),l?c.setAttribute("hidden",""):c.removeAttribute("hidden"))},{signal:n})}),e.querySelectorAll(".filter__checkbox").forEach(a=>{a.addEventListener("change",()=>{this.#j(),s?this.#z():this.#Z()},{signal:n})}),e.querySelectorAll(".filter__custom-select").forEach(a=>{a.addEventListener("click",l=>{if(l.target.closest(".filter__chip-remove"))return;const h=a.getAttribute("aria-expanded")==="true";if(e.querySelectorAll(".filter__custom-select").forEach(c=>{c.setAttribute("aria-expanded","false")}),a.setAttribute("aria-expanded",h?"false":"true"),!h){this.#J(a);const c=a.querySelector(".filter__option");c&&setTimeout(()=>c.focus(),0)}},{signal:n}),a.addEventListener("keydown",l=>{if(l.key==="Enter"||l.key===" "){l.preventDefault();const h=a.getAttribute("aria-expanded")==="true";if(a.setAttribute("aria-expanded",h?"false":"true"),!h){this.#J(a);const c=a.querySelector(".filter__option");c&&setTimeout(()=>c.focus(),0)}}else if(l.key==="Escape")a.setAttribute("aria-expanded","false"),a.focus();else if(l.key==="ArrowDown"){l.preventDefault();const h=[...a.querySelectorAll(".filter__option")],c=h.indexOf(this.#t.activeElement);(h[c+1]||h[0])?.focus()}else if(l.key==="ArrowUp"){l.preventDefault();const h=[...a.querySelectorAll(".filter__option")],c=h.indexOf(this.#t.activeElement);(h[c-1]||h[h.length-1])?.focus()}},{signal:n})}),e.querySelectorAll(".filter__option").forEach(a=>{a.addEventListener("click",l=>{const c=l.target.closest(".filter__section").dataset.property,u=l.target.dataset.value;this.#i[c]||(this.#i[c]=[]),this.#i[c].push(u),this.#k(),this.#T(),s?this.#z():this.#Z()},{signal:n}),a.addEventListener("keydown",l=>{l.key==="Enter"&&(l.preventDefault(),a.click())},{signal:n})}),e.querySelectorAll(".filter__chip-remove").forEach(a=>{a.addEventListener("click",l=>{l.stopPropagation();const h=l.target.closest(".filter__chip"),u=l.target.closest(".filter__section").dataset.property,m=h.dataset.value;this.#i[u]&&(this.#i[u]=this.#i[u].filter(v=>v!==m),this.#i[u].length===0&&delete this.#i[u]),this.#k(),this.#T(),s?this.#z():this.#Z()},{signal:n})}),this.#t.addEventListener("click",a=>{a.composedPath().some(l=>l?.classList?.contains("filter__custom-select"))||e.querySelectorAll(".filter__custom-select").forEach(l=>{l.setAttribute("aria-expanded","false")})},{signal:n}),e.querySelectorAll(".filter__range-input").forEach(a=>{const l=a.closest(".filter__section")?.dataset.property||"unknown",h=this.#nt(()=>{this.#j(),s?this.#z():this.#Z()},150,`range-${l}-${a.dataset.bound}`);a.addEventListener("input",c=>{const u=c.target.closest("label").querySelector("output");u&&(u.textContent=c.target.value),h()},{signal:n}),a.addEventListener("change",()=>{this.#j(),s?this.#z():this.#Z()},{signal:n})})}#j(){const e={};this.#t.querySelectorAll(".filter__section").forEach(n=>{const s=n.dataset.property,a=n.querySelectorAll(".filter__checkbox");if(a.length>0){const c=Array.from(a).filter(u=>u.checked).map(u=>u.value);c.length>0&&(e[s]=c);return}const l=n.querySelectorAll(".filter__chip");if(l.length>0){const c=Array.from(l).map(u=>u.dataset.value);c.length>0&&(e[s]=c);return}if(n.querySelectorAll(".filter__range-input").length===2){const c=n.querySelector('[data-bound="min"]'),u=n.querySelector('[data-bound="max"]');c&&u&&(e[s]={min:Number(c.value),max:Number(u.value)})}}),this.#i=e,this.#d=!1}#Z(){const e=this.#t.querySelector(".filter__apply");if(e){const n=JSON.stringify(this.#i)!==JSON.stringify(this.#a);e.classList.toggle("filter__apply--dirty",n)}}#z(){const e=new Map;this.#o.forEach((a,l)=>{const h=this.#i[l];a.sources.forEach(({element:c,property:u})=>{e.has(c)||e.set(c,{}),h!==void 0&&(e.get(c)[u]=h)})}),e.forEach((a,l)=>{l.dispatchEvent(new CustomEvent("dm-filter:apply",{bubbles:!1,detail:{filters:a}}))}),this.#a=JSON.parse(JSON.stringify(this.#i)),this.#tt();const n=this.#t.querySelector(".filter__apply");n&&n.classList.remove("filter__apply--dirty");const s=Object.keys(this.#i).length;this.#nt(()=>{this.#rt(s>0?`${s} filter${s!==1?"s":""} actief`:"Alle filters gewist")},500,"announce")()}#K(){this.#i={},this.#d=!1;const e=new Set;this.#o.forEach(s=>{s.sources.forEach(({element:a})=>e.add(a))}),e.forEach(s=>{s.dispatchEvent(new CustomEvent("dm-filter:apply",{bubbles:!1,detail:{filters:{}}}))}),this.#a={},this.#tt(),this.#rt("Alle filters zijn gewist"),this.#k(),this.#T();const n=this.#t.querySelector(".filter__apply");n&&n.classList.remove("filter__apply--dirty")}#st(e,n){const s=this.#i[e];return!s||!Array.isArray(s)?!1:s.includes(String(n))}#tt(){try{const e=new URL(window.location.href);Object.keys(this.#i).length===0?e.searchParams.delete("filters"):e.searchParams.set("filters",this.#at(this.#i)),window.history.pushState({},"",e)}catch{}}#et(e=!1){try{const n=new URLSearchParams(window.location.search).get("filters");if(!n)return!1;const s=this.#ot(n);return this.#i=s,e&&this.#z(),!0}catch{return!1}}#at(e){return btoa(encodeURIComponent(JSON.stringify(e)))}#ot(e){if(e.length>1e4)return{};const n=decodeURIComponent(atob(e));return this.#lt(JSON.parse(n))}#lt(e){if(typeof e!="object"||e===null||Array.isArray(e))return{};const n={};for(const[s,a]of Object.entries(e))Array.isArray(a)&&a.every(l=>typeof l=="string")?n[s]=a:a&&typeof a=="object"&&!Array.isArray(a)&&typeof a.min=="number"&&typeof a.max=="number"&&(n[s]={min:a.min,max:a.max});return n}#it(e,n){const s=n-e;return s<=10?.1:s<=100?1:s<=1e3?10:100}#I(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}#M(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}#ht(e){const n=e.getAttribute("filter-icons");if(!n)return null;try{return JSON.parse(n)}catch{return null}}#$(){return`<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
			<circle cx="10" cy="10" r="7" stroke="currentColor" stroke-width="1.5" fill="none"/>
		</svg>`}#ct(e){if(!e||typeof e!="string")return this.#$();try{const n=new DOMParser().parseFromString(e,"image/svg+xml");if(n.querySelector("parsererror"))return this.#$();const s=n.querySelector("svg");return s?(s.querySelectorAll("script, foreignObject, object, embed, iframe").forEach(a=>a.remove()),s.querySelectorAll("*").forEach(a=>{for(const l of[...a.attributes])(l.name.startsWith("on")||l.name==="href"||l.name==="xlink:href"||l.name.toLowerCase().includes("javascript"))&&a.removeAttribute(l.name)}),s.querySelectorAll("[href], [xlink\\:href]").forEach(a=>{const l=a.getAttribute("href")||a.getAttribute("xlink:href");l&&l.toLowerCase().includes("javascript:")&&a.remove()}),s.outerHTML):this.#$()}catch{return this.#$()}}#nt(e,n=150,s="default"){return(...a)=>{this.#s.has(s)&&clearTimeout(this.#s.get(s));const l=setTimeout(()=>{e.apply(this,a),this.#s.delete(s)},n);this.#s.set(s,l)}}#rt(e){const n=this.#t.querySelector('[role="status"]');n&&(n.textContent="",setTimeout(()=>{n.textContent=e},100))}get activeFilterCount(){return Object.keys(this.#i).length}getFilterState(){return JSON.parse(JSON.stringify(this.#i))}resetFilters(){this.#K()}exportFilters(){return JSON.stringify(this.#i)}disconnectedCallback(){this.#s.forEach(e=>clearTimeout(e)),this.#s.clear(),this.#m?.abort(),this.#m=null,this.#c&&(document.removeEventListener("click",this.#c,!0),this.#c=null),this.#y&&(window.removeEventListener("popstate",this.#y),this.#y=null),this.#e=[],this.#i={},this.#a={},this.#l.clear(),this.#o.clear(),this.#n=null}}return customElements.define("dm-filter",cu),Ge.ICON_SPRITE=jd,Ge.registerIcons=Ud,Ge.setIconSprite=Hd,Object.defineProperty(Ge,Symbol.toStringTag,{value:"Module"}),Ge})({});
