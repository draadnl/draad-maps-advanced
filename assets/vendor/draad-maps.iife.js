...
`;class tu extends HTMLElement{#t;#n=null;#e;#i=null;#a=null;#l=null;#r=-1;#s=[];#o=null;#h=null;#d;#u;static get observedAttributes(){return["placeholder","label","search-properties","search-labels","radius","min-chars","debounce","address-filter"]}attributeChangedCallback(e,n,s){n!==s&&this.#n&&(this.#p(),Ut(this.#t),this.#_())}constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#d=`search-input-${Math.random().toString(36).slice(2,9)}`,this.#u=`search-listbox-${Math.random().toString(36).slice(2,9)}`,this.#e=this.#c()}#c(){const e=new CSSStyleSheet;e.replaceSync(Yd),this.#t.adoptedStyleSheets=[e]}get placeholder(){return this.getAttribute("placeholder")||"Zoeken..."}set placeholder(e){this.setAttribute("placeholder",e)}get label(){return this.getAttribute("label")||"Zoek op straat, wijk of stadsdeel"}set label(e){this.setAttribute("label",e)}get searchProperties(){const e=this.getAttribute("search-properties");return e?e.split(",").map(n=>n.trim()).filter(n=>n):null}set searchProperties(e){Array.isArray(e)?this.setAttribute("search-properties",e.join(",")):this.setAttribute("search-properties",e)}get searchLabels(){const e=this.getAttribute("search-labels");return e?e.split(",").map(n=>n.trim()).filter(n=>n):null}set searchLabels(e){Array.isArray(e)?this.setAttribute("search-labels",e.join(",")):this.setAttribute("search-labels",e)}get radius(){const e=this.getAttribute("radius"),n=parseInt(e,10);return Number.isFinite(n)&&n>0?n:1e4}set radius(e){this.setAttribute("radius",String(e))}get minChars(){const e=this.getAttribute("min-chars"),n=parseInt(e,10);return Number.isFinite(n)&&n>=0?n:2}set minChars(e){this.setAttribute("min-chars",String(e))}get debounceDelay(){const e=this.getAttribute("debounce"),n=parseInt(e,10);return Number.isFinite(n)&&n>=0?n:300}set debounceDelay(e){this.setAttribute("debounce",String(e))}get addressFilter(){const e=this.getAttribute("address-filter");return e&&e.trim()?e.trim():null}set addressFilter(e){this.setAttribute("address-filter",e)}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#f(e):e.addEventListener("dm-map:ready",()=>{this.#f(e)},{once:!0}))}async#f(e){this.#n=e,await this.#e,this.#p(),Ut(this.#t),this.#_()}#p(){this.#t.innerHTML=`
				...
				<div class="search__form" part="form" role="search">
					<div class="search__input-wrapper" part="input-wrapper">
						<input
							...
							class="search__input"
							part="input"
							placeholder="${this.#R(this.placeholder)}"
							maxlength="200"
							autocomplete="off"
		...
		`}#_(){const e=this.#t.querySelector(".search__input"),n=this.#t.querySelector(".search__button"),s=this.#t.querySelector(".search__clear"),a=this.#t.querySelector(".search__listbox");e.addEventListener("input",l=>this.#y(l)),e.addEventListener("keydown",l=>this.#A(l)),e.addEventListener("focus",()=>this.#m()),e.addEventListener("blur",l=>this.#v(l)),n.addEventListener("click",()=>this.#k()),s.addEventListener("click",()=>{e.value="",s.hidden=!0,this.#s=[],this.#D(),e.focus()}),a.addEventListener("mousedown",l=>{l.preventDefault()}),a.addEventListener("click",l=>{const h=l.target.closest('[role="option"]');if(h){const c=parseInt(h.dataset.index,10);this.#F(c)}})}#y(e){const n=e.target.value.trim(),s=this.#t.querySelector(".search__clear");if(s&&(s.hidden=!e.target.value),this.#l&&(clearTimeout(this.#l),this.#l=null),this.#$(),n.length<this.minChars){this.#s=[],this.#D();return}this.#l=setTimeout(()=>{this.#g(n)},this.debounceDelay)}#A(e){const s=!this.#t.querySelector(".search__listbox").hidden;switch(e.key){case"ArrowDown":e.preventDefault(),s&&this.#N(1);break;case"ArrowUp":e.preventDefault(),s&&this.#N(-1);break;case"Enter":if(e.preventDefault(),s&&this.#s.length>0){const a=this.#r>=0?this.#r:0;this.#F(a)}else this.#E();break;case"Escape":e.preventDefault(),this.#D();break;case"Tab":this.#D();break}}#m(){this.#s.length>0&&this.#J()}#v(e){this.#a=setTimeout(()=>{this.#D()},150)}#k(){if(this.#s.length>0){const e=this.#r>=0?this.#r:0;this.#F(e)}else this.#E()}#E(){const n=this.#t.querySelector(".search__input").value.trim();n.length>=this.minChars&&this.#g(n)}async#g(e){this.#i&&this.#i.abort(),this.#i=new AbortController,this.#G(!0);try{const[n,s]=await Promise.all([this.#T(e),this.#C(e,this.#i.signal)]),a=s?.error===!0,l=a?s.results:s;this.#s=[...n.slice(0,5),...l.slice(0,10)],this.#q(a),this.#J();const h=this.#s.length;let c=h>0?`${h} resultaat${h!==1?"en":""} gevonden`:"Geen resultaten gevonden";a&&(c+=". Zoeken in adressen niet beschikbaar"),this.#H(c)}catch(n){n.name!=="AbortError"&&this.#H("Er is een fout opgetreden bij het zoeken")}finally{this.#G(!1)}}async#C(e,n){const s=new URL("https://api.pdok.nl/bzk/locatieserver/search/v3_1/free");s.searchParams.set("q",e),s.searchParams.set("rows","10"),s.searchParams.set("fq","type:(provincie OR gemeente OR woonplaats OR wijk OR buurt OR weg OR adres)"),this.addressFilter&&s.searchParams.append("fq",this.addressFilter),s.searchParams.set("fl","id,weergavenaam,type,score,centroide_ll,geometrie_ll");try{const a=await fetch(s.toString(),{signal:n});if(!a.ok)throw new Error(`HTTP ${a.status}`);return((await a.json()).response?.docs||[]).map(c=>({type:"address",label:c.weergavenaam,subtype:c.type,coordinates:this.#M(c.centroide_ll),geometry:c.geometrie_ll?this.#L(c.geometrie_ll):null,bounds:null,raw:c}))}catch(a){if(a.name==="AbortError")throw a;return{error:!0,results:[]}}}#T(e){const n=[],s=e.toLowerCase(),a=this.searchProperties,l=this.#n.querySelectorAll("dm-geojson, dm-wfs");for(const c of l){if(n.length>=5)break;const u=c.data;if(!u?.features)continue;const m=c.geoJsonLayer||c.wfsLayer;if(m)for(const v of u.features){if(n.length>=5)break;const _=v.properties||{},g=this.#P(_,s,a);if(g.length>0){const{label:w,matchedProperty:x}=this.#w(_,g);n.push({type:"feature",label:w,subtype:x,feature:v,layer:m,bounds:this.#x(v,m),coordinates:this.#b(v,m),source:c})}}}const h=this.#n.querySelectorAll("dm-marker[label]");for(const c of h){if(n.length>=5)break;const u=c.label;if(u&&u.toLowerCase().includes(s)){const m=c.center;if(!m)continue;n.push({type:"marker",label:u,subtype:"marker",coordinates:m,bounds:null,marker:c})}}return n}#P(e,n,s){const a=[];for(const[l,h]of Object.entries(e))s&&!s.includes(l)||typeof h=="string"&&h.toLowerCase().includes(n)&&a.push({key:l,value:h});return a}#w(e,n){const s=n[0],a=this.searchLabels,l=this.searchProperties;let h=s.key;if(l&&a){const c=l.indexOf(s.key);c>=0&&a[c]&&(h=a[c])}return{label:s.value,matchedProperty:h}}#x(e,n){const s=e.geometry;if(!s||s.type==="Point")return null;try{let l=null;if(n.eachLayer(h=>{h.feature===e&&(l=h)}),l&&l.getBounds)return l.getBounds()}catch{}return null}#b(e,n){const s=e.geometry;if(!s)return null;const a=s.type,l=s.coordinates;if(a==="Point")return[l[1],l[0]];try{let h=null;if(n.eachLayer(c=>{c.feature===e&&(h=c)}),h){if(h.getBounds){const c=h.getBounds().getCenter();return[c.lat,c.lng]}if(h.getLatLng){const c=h.getLatLng();return[c.lat,c.lng]}}}catch{}return null}#M(e){if(!e)return null;const n=e.match(/POINT\s*\(\s*([\d.-]+)\s+([\d.-]+)\s*\)/i);if(!n)return null;const s=parseFloat(n[1]),a=parseFloat(n[2]);return isNaN(a)||isNaN(s)?null:[a,s]}#L(e){if(!e)return null;const n=e.match(/^(\w+)/i);if(!n)return null;const s=n[1].toUpperCase();try{if(s==="POLYGON"||s==="MULTIPOLYGON")return this.#B(e,s);if(s==="LINESTRING"||s==="MULTILINESTRING")return this.#I(e,s);if(s==="POINT"){const a=this.#M(e);return a?{type:"Point",coordinates:[a[1],a[0]]}:null}}catch{}return null}#B(e,n){const s=[],a=/\(\s*([\d.\s,-]+)\s*\)/g;let l;for(;(l=a.exec(e))!==null;){const h=l[1].split(",").map(c=>{const[u,m]=c.trim().split(/\s+/).map(parseFloat);return[m,u]});h.length>0&&s.push(h)}return s.length===0?null:{type:n==="MULTIPOLYGON"?"MultiPolygon":"Polygon",coordinates:s}}#I(e,n){const s=[];if(n==="LINESTRING"){const a=e.match(/LINESTRING\s*\(\s*([\d.\s,-]+)\s*\)/i);if(a){const l=a[1].split(",").map(h=>{const[c,u]=h.trim().split(/\s+/).map(parseFloat);return[u,c]});s.push(l)}}else{const a=/\(\s*([\d.\s,-]+)\s*\)/g;let l;for(;(l=a.exec(e))!==null;){const h=l[1].split(",").map(c=>{const[u,m]=c.trim().split(/\s+/).map(parseFloat);return[m,u]});h.length>0&&s.push(h)}}return s.length===0?null:{type:n==="MULTILINESTRING"?"MultiLineString":"LineString",coordinates:s}}#U(e){if(e.type==="address"&&e.subtype){const n=e.subtype.toLowerCase();if(n==="provincie")return 5e4;if(n==="gemeente")return 2e4;if(n==="woonplaats")return 15e3;if(n==="wijk"||n==="buurt")return 3e3;if(n==="weg")return 5e3}return this.radius}#q(e=!1){const n=this.#t.querySelector(".search__listbox"),s=this.#t.querySelector(".search__input");if(this.#s.length===0){n.innerHTML=`
			...
			`};let m="";c?(m+='<li class="search__group-label" part="group-label" role="presentation">Op de kaart</li>',m+=l.map(v=>u(v,this.#s.indexOf(v))).join(""),m+='<li class="search__group-label" part="group-label" role="presentation">Adressen</li>',m+=h.map(v=>u(v,this.#s.indexOf(v))).join("")):m+=this.#s.map((v,_)=>u(v,_)).join(""),n.innerHTML=m+a,this.#r>=0?s.setAttribute("aria-activedescendant",`search-option-${this.#r}`):s.removeAttribute("aria-activedescendant")}#V(e){switch(e.type){case"address":return e.subtype||"Adres";case"feature":return e.subtype||"Object";case"marker":return"Marker";default:return""}}#X(e){switch(e.type){case"address":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-map-pin"/></svg>';case"feature":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-hamburger"/></svg>';case"marker":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-map-pin"/></svg>';default:return""}}#J(){const e=this.#t.querySelector(".search__listbox"),n=this.#t.querySelector(".search__input"),s=this.#t.querySelector(".search__form");e.hidden=!1,n.setAttribute("aria-expanded","true"),s.dataset.open="true"}#D(){const e=this.#t.querySelector(".search__listbox"),n=this.#t.querySelector(".search__input"),s=this.#t.querySelector(".search__form");e.hidden=!0,n.setAttribute("aria-expanded","false"),n.removeAttribute("aria-activedescendant"),this.#r=-1,delete s.dataset.open}#N(e){const n=this.#s.length-1;if(n<0)return;this.#r+=e,this.#r<0?this.#r=n:this.#r>n&&(this.#r=0),this.#q();const s=this.#t.querySelector(`#search-option-${this.#r}`);s&&s.scrollIntoView({block:"nearest"})}#F(e){const n=this.#s[e];if(!n)return;const s=this.#t.querySelector(".search__input"),a=this.#t.querySelector(".search__clear");s.value=n.label,a&&(a.hidden=!n.label),this.#D(),this.#K(n),this.#Y(n),this.dispatchEvent(new CustomEvent("dm-search:select",{bubbles:!0,composed:!0,detail:{type:n.type,label:n.label,coordinates:n.coordinates,bounds:n.bounds,feature:n.feature,marker:n.marker,raw:n.raw}}))}#K(e){const n=this.#n.map;if(!n)return;this.#$();const s=e.geometry?.type||"";s.includes("Polygon")||s.includes("Line")?this.#Q(e):e.type==="address"&&e.coordinates&&this.#O(e.coordinates);let a;if(this.#o?.getBounds)a=this.#o.getBounds();else if(e.bounds)a=e.bounds;else if(e.coordinates){const l=G.latLng(e.coordinates[0],e.coordinates[1]),h=this.#U(e);a=l.toBounds(h)}a&&(n.fitBounds(a,{padding:[40,40]}),this.dispatchEvent(new CustomEvent("dm-search:zoom",{bubbles:!0,composed:!0,detail:{bounds:a,result:e}})))}#Q(e){const n=this.#n.map;if(!n||!e.geometry)return;const s=e.geometry,a=s.type==="LineString"||s.type==="MultiLineString",l=this.#j(),h={color:l,weight:a?4:2,opacity:.8,fillColor:l,fillOpacity:a?0:.15};s.type==="Polygon"||s.type==="MultiPolygon"?this.#o=G.polygon(s.coordinates,h):s.type==="LineString"?this.#o=G.polyline(s.coordinates[0],h):s.type==="MultiLineString"&&(this.#o=G.polyline(s.coordinates,h)),this.#o&&this.#o.addTo(n)}#O(e){const n=this.#n?.map;!n||!e||(this.#o=G.circleMarker(e,{radius:9,color:"#fff",weight:2,fillColor:this.#j(),fillOpacity:1,opacity:1}).addTo(n))}#j(){const e=getComputedStyle(this);return e.getPropertyValue("--draad-map-components-search-highlight-color").trim()||e.getPropertyValue("--draad-map-components-color-primary").trim()||"#268641"}#Y(e){if(e.type==="marker"&&e.marker)e.marker.active=!0,e.marker.marker?.setZIndexOffset?.(1e3),this.#h=e.marker,e.marker.dispatchEvent(new CustomEvent("dm-marker:click",{bubbles:!0,composed:!0,detail:{marker:e.marker.marker??null,targetId:e.marker.target}}));else if(e.type==="feature"&&e.source){const n=e.source.tagName==="DM-WFS"?"wfs":"geojson";e.source.dispatchEvent(new CustomEvent(`dm-${n}:feature-click`,{bubbles:!0,composed:!0,detail:{feature:e.feature,layer:e.layer,latlng:null}}))}}#$(){this.#o&&this.#n?.map&&(this.#n.map.removeLayer(this.#o),this.#o=null),this.#n?.querySelectorAll("dm-geojson, dm-wfs").forEach(e=>e.clearActiveFeature?.()),this.#h&&(this.#h.active=!1,this.#h.marker?.setZIndexOffset?.(0),this.#h=null)}#G(e){const n=this.#t.querySelector(".search__button");e?(n.classList.add("search__button--loading"),n.setAttribute("aria-busy","true")):(n.classList.remove("search__button--loading"),n.removeAttribute("aria-busy"))}#H(e){const n=this.#t.querySelector('[role="status"]');n&&(n.textContent="",setTimeout(()=>{n.textContent=e},100))}#Z(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}#R(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}disconnectedCallback(){this.#i&&(this.#i.abort(),this.#i=null),this.#a&&(clearTimeout(this.#a),this.#a=null),this.#l&&(clearTimeout(this.#l),this.#l=null),this.#$(),this.#n=null,this.#s=[]}}customElements.define("dm-search",tu);const eu=`/* dm-infowindow - Card panel styling */
:host {
	display: flex;
	flex-direction: column;
	max-block-size: 100%;
...
`;class iu extends HTMLElement{#t;#n=null;#e=null;#i=null;#a=null;#l=null;#r=null;#s=null;#o=null;#h=null;#d=!1;#u;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#u=this.#c()}#c(){const e=new CSSStyleSheet;e.replaceSync(eu),this.#t.adoptedStyleSheets=[e]}static get observedAttributes(){return["for","feature-id","trigger"]}get for(){return this.getAttribute("for")}set for(e){this.setAttribute("for",e)}get featureId(){return this.getAttribute("feature-id")}set featureId(e){this.setAttribute("feature-id",e)}get trigger(){return this.getAttribute("trigger")||"click"}set trigger(e){this.setAttribute("trigger",e)}get isOpen(){return this.#d}connectedCallback(){const e=this.closest("dm-map");e&&(this.#n=e,this.setAttribute("slot","top-right"),this.#_(),this.setAttribute("hidden",""),Ut(this.#t),e.map?this.#f():e.addEventListener("dm-map:ready",()=>{this.#f()},{once:!0}))}async#f(){await this.#u,this.#y(),this.#l=e=>this.#p(e),this.#n.addEventListener("dm-infowindow:open",this.#l)}#p(e){e.target!==this&&this.isOpen&&this.close()}#_(){this.#t.innerHTML=`
		...
		`,this.#t.querySelector(".infowindow__close").addEventListener("click",()=>this.close())}#y(){const e=this.for;e&&(this.#e=document.getElementById(e),this.#e?this.#m():this.#A(e))}#A(e){this.#s&&this.#s.disconnect(),this.#s=new MutationObserver((n,s)=>{const a=document.getElementById(e);a&&(this.#e=a,this.#m(),s.disconnect(),this.#s=null)}),this.#s.observe(this.#n,{childList:!0,subtree:!0})}#m(){const e=this.trigger,n=this.#e.tagName.toLowerCase();n==="dm-marker"?this.#k(e):(n==="dm-geojson"||n==="dm-wfs")&&this.#E(e,n)}#v(){if(this.#e&&(this.#i&&(this.#e.removeEventListener("dm-marker:click",this.#i),this.#i=null),this.#a)){const e=this.#e.tagName.toLowerCase().replace("dm-","");this.#e.removeEventListener(`dm-${e}:feature-click`,this.#a),this.#a=null}}#k(e){e==="click"&&(this.#i=n=>this.#g(n),this.#e.addEventListener("dm-marker:click",this.#i))}#E(e,n){const a=`dm-${n.replace("dm-","")}:feature-click`;e==="click"&&(this.#a=l=>this.#C(l),this.#e.addEventListener(a,this.#a))}#g(e){if(this.#h=null,this.isOpen)this.close();else{const n=this.#P();this.#U(n),this.open()}}#C(e){const{feature:n}=e.detail;if(!this.#T(n))return;this.#h=n;const s=this.#w(n);this.#U(s),this.#L(this,n),this.open()}#T(e){const n=this.featureId;return!n||n==="*"||e.id!==void 0&&String(e.id)===n||e.properties?.id!==void 0&&String(e.properties.id)===n}#P(){const e=document.createElement("div"),n=this.querySelector("template");if(n)e.appendChild(n.content.cloneNode(!0));else for(const s of this.childNodes)s.nodeType===Node.ELEMENT_NODE&&s.hasAttribute("slot")||e.appendChild(s.cloneNode(!0));return this.#b(e),e}#w(e){const n=document.createElement("div"),s=this.querySelector("template");if(s){const a=s.content.cloneNode(!0);n.appendChild(a),this.#L(n,e),this.#B(n)}else if(this.#e&&this.#x()){const a=this.#M(e);a&&n.appendChild(a)}else for(const a of this.childNodes)a.nodeType===Node.ELEMENT_NODE&&a.hasAttribute("slot")||n.appendChild(a.cloneNode(!0));return this.#b(n),n}#x(){return this.#e?.getAttribute("infowindow-properties")}#b(e){const n=a=>{const l=document.createElementNS("http://www.w3.org/2000/svg","svg");l.setAttribute("aria-hidden","true");const h=document.createElementNS("http://www.w3.org/2000/svg","use");return h.setAttribute("href",`#${a}`),l.appendChild(h),l},s=a=>{const l=a.getAttribute("href");if(!l)return!1;try{return new URL(l,window.location.href).origin!==window.location.origin}catch{return!1}};e.querySelectorAll(".address, address").forEach(a=>{a.setAttribute("part","address"),a.prepend(n("icon-map-pin"))}),e.querySelectorAll(".action, a.button, button.action").forEach(a=>{a.setAttribute("part","action"),s(a)&&a.prepend(n("icon-external-link"))})}#M(e){const n=this.#e.getAttribute("infowindow-properties"),s=this.#e.getAttribute("infowindow-labels");if(!n)return null;const a=n.split(",").map(c=>c.trim()),l=s?s.split(",").map(c=>c.trim()):a,h=document.createElement("dl");h.className="dm-infowindow-data",h.setAttribute("part","data-table");for(let c=0;c<a.length;c++){const u=e.properties?.[a[c]];if(u==null||u==="")continue;const m=document.createElement("dt");m.textContent=l[c]||a[c],h.appendChild(m);const v=document.createElement("dd");v.innerHTML=u!==null&&typeof u=="object"?JSON.stringify(u):String(u),h.appendChild(v)}return h.children.length>0?h:null}#L(e,n){const s=new Set(["https:","http:","mailto:","tel:"]),a=(l,h)=>{l.style.display=h?"":"none"};e.querySelectorAll("[data-field]").forEach(l=>{const h=this.#I(n,l.getAttribute("data-field")),c=h!=null?String(h):"";l.textContent=c,a(l,c!=="")}),e.querySelectorAll("[data-href]").forEach(l=>{const h=this.#I(n,l.getAttribute("data-href"));let c=!1;if(h!=null)try{const u=new URL(String(h),window.location.origin);s.has(u.protocol)?(l.setAttribute("href",String(h)),c=!0):l.removeAttribute("href")}catch{l.removeAttribute("href")}else l.removeAttribute("href");a(l,c)}),e.querySelectorAll("[data-src]").forEach(l=>{const h=this.#I(n,l.getAttribute("data-src"));let c=!1;if(h!=null)try{const u=new URL(String(h),window.location.origin);u.protocol==="https:"||u.protocol==="http:"?(l.setAttribute("src",String(h)),c=!0):l.removeAttribute("src")}catch{l.removeAttribute("src")}else l.removeAttribute("src");a(l,c)})}#B(e){e.querySelectorAll("dl.dm-infowindow-data").forEach(n=>{const s=Array.from(n.children);let a=!1;s.forEach((l,h)=>{if(l.tagName!=="DD")return;if(l.style.display==="none"||l.textContent===""){l.style.display="none";const u=s[h-1];u&&u.tagName==="DT"&&(u.style.display="none")}else a=!0}),a||(n.style.display="none")})}#I(e,n){if(!n.startsWith("properties.")&&n!=="id"&&n!=="properties")return;const s=new Set(["__proto__","constructor","prototype"]),a=n.split(".");let l=e;for(const h of a){if(l==null||s.has(h)||!Object.prototype.hasOwnProperty.call(l,h))return;l=l[h]}return l}#U(e){const n=this.#t.querySelector(".infowindow__content");for(n.innerHTML="";e.firstChild;)n.appendChild(e.firstChild);n.querySelectorAll("a").forEach(s=>s.setAttribute("part","link"))}open(){this.#t.querySelector(".infowindow").removeAttribute("hidden"),this.removeAttribute("hidden"),this.#d=!0,this.#q(),this.dispatchEvent(new CustomEvent("dm-infowindow:open",{bubbles:!0,composed:!0,detail:{target:this.#e,feature:this.#h}}))}close(){this.#t.querySelector(".infowindow").setAttribute("hidden",""),this.setAttribute("hidden",""),this.#d=!1,this.#r&&(document.removeEventListener("keydown",this.#r),this.#r=null),this.#e?.tagName.toLowerCase()==="dm-marker"?this.#e.active=!1:this.#e?.clearActiveFeature?.(),this.#o&&typeof this.#o.focus=="function"&&this.#o.focus(),this.#o=null,this.dispatchEvent(new CustomEvent("dm-infowindow:close",{bubbles:!0,composed:!0,detail:{target:this.#e}}))}toggle(){this.isOpen?this.close():this.open()}#q(){const e=this.#t.querySelector(".infowindow");this.#o=document.activeElement,e.setAttribute("tabindex","-1"),e.focus(),this.#r=n=>{n.key==="Escape"&&this.isOpen&&this.close()},document.addEventListener("keydown",this.#r)}attributeChangedCallback(e,n,s){if(!(n===s||!this.#n))switch(e){case"for":this.#v(),this.#e=null,this.#y();break;case"trigger":this.#v(),this.#e&&this.#m();break}}disconnectedCallback(){this.#v(),this.#l&&(this.#n?.removeEventListener("dm-infowindow:open",this.#l),this.#l=null),this.#r&&(document.removeEventListener("keydown",this.#r),this.#r=null),this.#s&&(this.#s.disconnect(),this.#s=null),this.#n=null,this.#e=null,this.#o=null,this.#h=null}}customElements.define("dm-infowindow",iu);const nu=`.icon {
	display: block;
	inline-size: 1em;
	block-size: 1em;
	fill: currentColor;
...
`;class cu extends HTMLElement{#t;#n=null;#e=[];#i={};#a={};#l=new Map;#r=new WeakMap;#s=new Map;#o=new Map;#h;#d=!1;#u=!1;#c=null;#f=null;#p=null;#_=!1;#y=null;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#h=this.#A()}#A(){const e=new CSSStyleSheet;e.replaceSync(hu),this.#t.adoptedStyleSheets=[e]}get#m(){const e=this.getAttribute("variant");return e==="dropdown"||e==="sidebar"?e:"dialog"}get#v(){return this.getAttribute("submit")==="auto"}get#k(){return this.getAttribute("dismiss")==="close"?"close":"actions"}get#E(){return this.hasAttribute("collapsible")}get#g(){return this.getAttribute("bool-label")||"Ja"}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#C(e):e.addEventListener("dm-map:ready",()=>{this.#C(e)},{once:!0}))}async#C(e){if(this.#n=e,await this.#h,this.#T(),this.#P(),this.#U(),Ut(this.#t),this.#Y(),this.#N(),this.#G(),this.hasAttribute("open")&&this.#m==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__toggle");s?.classList.add("filter__panel--open"),a?.setAttribute("aria-expanded","true"),this.#u=!0}this.#it()&&(this.#d=!0,this.#a=JSON.parse(JSON.stringify(this.#i))),this.#y=()=>this.#it(!0),window.addEventListener("popstate",this.#y)}#T(){const e=["dm-geojson[filter-property], dm-geojson[filter-properties]","dm-wfs[filter-property], dm-wfs[filter-properties]","dm-layer[filter-property], dm-layer[filter-properties]"];this.#e=[],e.forEach(n=>{this.#n.querySelectorAll(n).forEach(s=>{const a=s.getAttribute("filter-properties"),l=s.getAttribute("filter-labels"),h=s.getAttribute("filter-property"),c=s.getAttribute("filter-label"),u=s.getAttribute("filter-types"),m=s.getAttribute("filter-collapsed"),v=s.getAttribute("filter-bool-labels");let _=[],g=[],w=[],x=[],S=[];if(a){for(_=a.split(",").map(E=>E.trim()).filter(Boolean),l&&(g=l.split(",").map(E=>E.trim()).filter(Boolean)),v&&(S=v.split(",").map(E=>E.trim())),u&&(w=u.split(",").map(E=>E.trim()).filter(Boolean)),m&&(x=m.split(",").map(E=>E.trim()).filter(Boolean));g.length<_.length;)g.push(_[g.length]);for(;w.length<_.length;)w.push("auto");for(;x.length<_.length;)x.push("false");for(;S.length<_.length;)S.push("")}else h&&(_=[h],g=[c||h],w=[u||"auto"],x=[m||"false"],S=[v||""]);if(_.length===0)return;const P=s.id||`filter-source-${this.#e.length}`,C=s.getAttribute("filter-key");if(_.forEach((E,z)=>{this.#e.push({element:s,label:g[z],property:E,groupKey:C||E,type:s.tagName.toLowerCase(),id:P,filterType:w[z]||"auto",collapsed:x[z]==="true",boolLabel:S[z]||""})}),!this.#r.has(s))if(this.#r.set(s,!0),s.data)_.forEach(E=>{this.#x(s,E)});else{const E=s.tagName==="DM-GEOJSON"?"dm-geojson:ready":s.tagName==="DM-WFS"?"dm-wfs:ready":"dm-layer:ready";let z=!1;s.addEventListener(E,()=>{z||(z=!0,_.forEach(R=>{this.#x(s,R)}),this.#P(),this.#N(),this.#G())},{once:!0}),s.data&&!z&&(z=!0,_.forEach(R=>{this.#x(s,R)}))}})})}#P(){this.#o=new Map,this.#e.forEach(e=>{this.#o.has(e.groupKey)||this.#o.set(e.groupKey,{label:e.label,filterType:e.filterType,collapsed:e.collapsed,boolLabel:e.boolLabel,sources:[]});const n=this.#o.get(e.groupKey);n.sources.find(a=>a.element===e.element&&a.property===e.property)||n.sources.push({element:e.element,id:e.id,property:e.property})})}#w(e,n){const s=new Set;if(n.sources.forEach(({id:l,property:h})=>{const c=`${l}:${h}`,u=this.#l.get(c);u&&u.values.forEach(m=>s.add(m))}),s.size===0)return null;const a=Array.from(s).sort((l,h)=>typeof l=="number"&&typeof h=="number"?l-h:String(l).localeCompare(String(h)));return{values:a,valueType:this.#L(a)}}#x(e,n){const s=this.#b(e,n),a=this.#L(s),l=this.#e.find(c=>c.element===e&&c.property===n);if(!l)return;const h=`${l.id}:${n}`;this.#l.set(h,{values:s,valueType:a}),this.#d&&Object.keys(this.#i).length>0&&this.#R(),this.#K()&&(this.#N(),this.#G())}#b(e,n){if(!e.data?.features)return[];const s=new Set;return e.data.features.forEach(a=>{const l=a.properties?.[n];if(l!=null){const h=String(l);h.includes(",")?h.split(",").forEach(c=>{const u=c.trim();u&&s.add(u)}):s.add(l)}}),Array.from(s).sort((a,l)=>typeof a=="number"&&typeof l=="number"?a-l:String(a).localeCompare(String(l)))}#M(e,n){const s=new Map;return n.sources.forEach(({element:a,property:l})=>{a.data?.features&&a.data.features.forEach(h=>{const c=h.properties?.[l];if(c==null)return;const u=String(c);(u.includes(",")?u.split(",").map(v=>v.trim()).filter(Boolean):[u]).forEach(v=>{s.set(v,(s.get(v)||0)+1)})})}),s}#L(e){if(e.length===0)return{isNumeric:!1};if(e.length===1)return{isNumeric:!1,single:!0};if(e.every(s=>typeof s=="number"?!0:typeof s=="string"?!isNaN(Number(s))&&s.trim()!=="":!1)){const s=e.map(a=>Number(a));return{isNumeric:!0,min:Math.min(...s),max:Math.max(...s)}}return{isNumeric:!1}}#B(e,n){const s=this.#o.get(e)?.filterType;return s&&s!=="auto"?s==="range"&&!n.valueType.isNumeric?this.#I(n):s:this.#I(n)}#I({values:e,valueType:n}){return n.isNumeric&&!n.single?"range":e.length<=20?"checkbox":"dropdown"}#U(){const e=this.#m;e==="dropdown"?this.#J():e==="sidebar"?this.#D():this.#X()}#q(){return`
			<button
				part="toggle"
		...
		`}#V(){const e=this.#v?" filter--auto-submit":"",n=this.getAttribute("label")||"Filters";return`
		...
		`}#X(){const e=this.getAttribute("label")||"Filters",n=this.#v?" filter--auto-submit":"";this.#t.innerHTML=`
		...
		`}#J(){const e=this.getAttribute("label")||"Filters",n=this.#v?" filter--auto-submit":"";this.#t.innerHTML=`
		...
		`}#D(){const e=this.getAttribute("label")||"Filters",n=this.#v?" filter--auto-submit":"";this.#t.innerHTML=`
		...
		`}#N(){if(!this.#_){this.#_=!0;try{const e=this.#t.querySelector(".filter__content");if(!e)return;if(this.#o.size===0){e.innerHTML='<p class="filter__empty">Geen filterbare gegevensbronnen gevonden.</p>';return}const n=[];this.#o.forEach((s,a)=>{let l=this.#w(a,s);if(l||(s.sources.forEach(({element:E,property:z})=>{E.data&&this.#x(E,z)}),l=this.#w(a,s)),!l||l.values.length===0){n.push(`
						<div class="filter__section" part="section" data-property="${this.#S(a)}">
							<h3 class="filter__section-title" part="section-title">${this.#z(s.label)}</h3>
							<p class="filter__loading">Loading data...</p>
						</div>
					`);return}const{values:h,valueType:c}=l,u=this.#B(a,l),m=s.collapsed,v=m?"false":"true",_=m?" hidden":"";if(u==="bool"){const E=this.#i[a]?.[0]==="1"?" checked":"",z=s.boolLabel||this.#g,R=this.#z(z),F=`
						<label class="filter__checkbox-label">
							<input
								type="checkbox"
								class="filter__checkbox"
								value="1"
								aria-label="${this.#z(`${s.label}: ${z}`)}"
								${E}
							>
					...
					`);return}if(u==="single"||c.single){n.push(`
						<div class="filter__section" part="section" data-property="${this.#S(a)}">
							${this.#F(s.label,a,v,_,`
								<p class="filter__single">Value: ${this.#z(String(h[0]))}</p>
							`)}
						</div>
					...
					`);return}if(u==="checkbox"){const E=s.sources.reduce((F,{element:J})=>F||this.#ct(J),null),z=this.#M(a,s),R=h.map(F=>{const J=this.#at(a,F)?"checked":"",ot=this.#S(String(F)),et=this.#z(String(F)),St=z.get(String(F))||0,Q=St>0?` (${St})`:"",ht=E?.[F]?this.#dt(E[F]):this.#W();return`
							<label class="filter__checkbox-label">
								<input
									type="checkbox"
									class="filter__checkbox"
						...
						<fieldset class="filter__fieldset">
							<legend class="visually-hidden">${this.#z(s.label)}</legend>
							${R}
						</fieldset>`;n.push(`
						<div class="filter__section" part="section" data-property="${this.#S(a)}">
							${this.#F(s.label,a,v,_,j)}
						</div>
					`);return}const g=this.#i[a]||[],w=g.map(E=>{const z=this.#S(String(E)),R=this.#z(String(E));return`
					...
					`}).join(""),x=h.filter(E=>!g.includes(String(E))).map(E=>{const z=this.#S(String(E)),R=this.#z(String(E));return`
				...
				<button type="button" part="section-toggle" class="filter__section-toggle" aria-expanded="${s}" aria-controls="${h}">
					<span class="filter__section-toggle-text">${this.#z(e)}</span>
