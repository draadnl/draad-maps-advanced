...
`;class tu extends HTMLElement{#t;#n=null;#e;#i=null;#a=null;#s=null;#l=-1;#r=[];#o=null;#h;#u;static get observedAttributes(){return["placeholder","label","search-properties","search-labels","radius","min-chars","debounce","address-filter"]}attributeChangedCallback(e,n,s){n!==s&&this.#n&&(this.#f(),Ut(this.#t),this.#_())}constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#h=`search-input-${Math.random().toString(36).slice(2,9)}`,this.#u=`search-listbox-${Math.random().toString(36).slice(2,9)}`,this.#e=this.#d()}#d(){const e=new CSSStyleSheet;e.replaceSync(Yd),this.#t.adoptedStyleSheets=[e]}get placeholder(){return this.getAttribute("placeholder")||"Zoeken..."}set placeholder(e){this.setAttribute("placeholder",e)}get label(){return this.getAttribute("label")||"Zoek op straat, wijk of stadsdeel"}set label(e){this.setAttribute("label",e)}get searchProperties(){const e=this.getAttribute("search-properties");return e?e.split(",").map(n=>n.trim()).filter(n=>n):null}set searchProperties(e){Array.isArray(e)?this.setAttribute("search-properties",e.join(",")):this.setAttribute("search-properties",e)}get searchLabels(){const e=this.getAttribute("search-labels");return e?e.split(",").map(n=>n.trim()).filter(n=>n):null}set searchLabels(e){Array.isArray(e)?this.setAttribute("search-labels",e.join(",")):this.setAttribute("search-labels",e)}get radius(){const e=this.getAttribute("radius"),n=parseInt(e,10);return Number.isFinite(n)&&n>0?n:1e4}set radius(e){this.setAttribute("radius",String(e))}get minChars(){const e=this.getAttribute("min-chars"),n=parseInt(e,10);return Number.isFinite(n)&&n>=0?n:2}set minChars(e){this.setAttribute("min-chars",String(e))}get debounceDelay(){const e=this.getAttribute("debounce"),n=parseInt(e,10);return Number.isFinite(n)&&n>=0?n:300}set debounceDelay(e){this.setAttribute("debounce",String(e))}get addressFilter(){const e=this.getAttribute("address-filter");return e&&e.trim()?e.trim():null}set addressFilter(e){this.setAttribute("address-filter",e)}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#c(e):e.addEventListener("dm-map:ready",()=>{this.#c(e)},{once:!0}))}async#c(e){this.#n=e,await this.#e,this.#f(),Ut(this.#t),this.#_()}#f(){this.#t.innerHTML=`
				...
				<div class="search__form" part="form" role="search">
					<div class="search__input-wrapper" part="input-wrapper">
						<input
							...
							class="search__input"
							part="input"
							placeholder="${this.#K(this.placeholder)}"
							maxlength="200"
							autocomplete="off"
		...
		`}#_(){const e=this.#t.querySelector(".search__input"),n=this.#t.querySelector(".search__button"),s=this.#t.querySelector(".search__clear"),a=this.#t.querySelector(".search__listbox");e.addEventListener("input",l=>this.#v(l)),e.addEventListener("keydown",l=>this.#y(l)),e.addEventListener("focus",()=>this.#A()),e.addEventListener("blur",l=>this.#p(l)),n.addEventListener("click",()=>this.#g()),s.addEventListener("click",()=>{e.value="",s.hidden=!0,this.#r=[],this.#k(),e.focus()}),a.addEventListener("mousedown",l=>{l.preventDefault()}),a.addEventListener("click",l=>{const h=l.target.closest('[role="option"]');if(h){const c=parseInt(h.dataset.index,10);this.#Z(c)}})}#v(e){const n=e.target.value.trim(),s=this.#t.querySelector(".search__clear");if(s&&(s.hidden=!e.target.value),this.#s&&(clearTimeout(this.#s),this.#s=null),this.#C(),n.length<this.minChars){this.#r=[],this.#k();return}this.#s=setTimeout(()=>{this.#M(n)},this.debounceDelay)}#y(e){const s=!this.#t.querySelector(".search__listbox").hidden;switch(e.key){case"ArrowDown":e.preventDefault(),s&&this.#F(1);break;case"ArrowUp":e.preventDefault(),s&&this.#F(-1);break;case"Enter":if(e.preventDefault(),s&&this.#r.length>0){const a=this.#l>=0?this.#l:0;this.#Z(a)}else this.#L();break;case"Escape":e.preventDefault(),this.#k();break;case"Tab":this.#k();break}}#A(){this.#r.length>0&&this.#W()}#p(e){this.#a=setTimeout(()=>{this.#k()},150)}#g(){if(this.#r.length>0){const e=this.#l>=0?this.#l:0;this.#Z(e)}else this.#L()}#L(){const n=this.#t.querySelector(".search__input").value.trim();n.length>=this.minChars&&this.#M(n)}async#M(e){this.#i&&this.#i.abort(),this.#i=new AbortController,this.#D(!0);try{const[n,s]=await Promise.all([this.#b(e),this.#w(e,this.#i.signal)]),a=s?.error===!0,l=a?s.results:s;this.#r=[...n.slice(0,5),...l.slice(0,10)],this.#G(a),this.#W();const h=this.#r.length;let c=h>0?`${h} resultaat${h!==1?"en":""} gevonden`:"Geen resultaten gevonden";a&&(c+=". Zoeken in adressen niet beschikbaar"),this.#B(c)}catch(n){n.name!=="AbortError"&&this.#B("Er is een fout opgetreden bij het zoeken")}finally{this.#D(!1)}}async#w(e,n){const s=new URL("https://api.pdok.nl/bzk/locatieserver/search/v3_1/free");s.searchParams.set("q",e),s.searchParams.set("rows","10"),s.searchParams.set("fq","type:(provincie OR gemeente OR woonplaats OR wijk OR buurt OR weg OR adres)"),this.addressFilter&&s.searchParams.append("fq",this.addressFilter),s.searchParams.set("fl","id,weergavenaam,type,score,centroide_ll,geometrie_ll");try{const a=await fetch(s.toString(),{signal:n});if(!a.ok)throw new Error(`HTTP ${a.status}`);return((await a.json()).response?.docs||[]).map(c=>({type:"address",label:c.weergavenaam,subtype:c.type,coordinates:this.#P(c.centroide_ll),geometry:c.geometrie_ll?this.#z(c.geometrie_ll):null,bounds:null,raw:c}))}catch(a){if(a.name==="AbortError")throw a;return{error:!0,results:[]}}}#b(e){const n=[],s=e.toLowerCase(),a=this.searchProperties,l=this.#n.querySelectorAll("dm-geojson, dm-wfs");for(const c of l){if(n.length>=5)break;const u=c.data;if(!u?.features)continue;const m=c.geoJsonLayer||c.wfsLayer;if(m)for(const v of u.features){if(n.length>=5)break;const g=v.properties||{},_=this.#S(g,s,a);if(_.length>0){const{label:w,matchedProperty:x}=this.#m(g,_);n.push({type:"feature",label:w,subtype:x,feature:v,layer:m,bounds:this.#E(v,m),coordinates:this.#I(v,m),source:c})}}}const h=this.#n.querySelectorAll("dm-marker[label]");for(const c of h){if(n.length>=5)break;const u=c.label;if(u&&u.toLowerCase().includes(s)){const m=c.center;if(!m)continue;n.push({type:"marker",label:u,subtype:"marker",coordinates:m,bounds:null,marker:c})}}return n}#S(e,n,s){const a=[];for(const[l,h]of Object.entries(e))s&&!s.includes(l)||typeof h=="string"&&h.toLowerCase().includes(n)&&a.push({key:l,value:h});return a}#m(e,n){const s=n[0],a=this.searchLabels,l=this.searchProperties;let h=s.key;if(l&&a){const c=l.indexOf(s.key);c>=0&&a[c]&&(h=a[c])}return{label:s.value,matchedProperty:h}}#E(e,n){const s=e.geometry;if(!s||s.type==="Point")return null;try{let l=null;if(n.eachLayer(h=>{h.feature===e&&(l=h)}),l&&l.getBounds)return l.getBounds()}catch{}return null}#I(e,n){const s=e.geometry;if(!s)return null;const a=s.type,l=s.coordinates;if(a==="Point")return[l[1],l[0]];try{let h=null;if(n.eachLayer(c=>{c.feature===e&&(h=c)}),h){if(h.getBounds){const c=h.getBounds().getCenter();return[c.lat,c.lng]}if(h.getLatLng){const c=h.getLatLng();return[c.lat,c.lng]}}}catch{}return null}#P(e){if(!e)return null;const n=e.match(/POINT\s*\(\s*([\d.-]+)\s+([\d.-]+)\s*\)/i);if(!n)return null;const s=parseFloat(n[1]),a=parseFloat(n[2]);return isNaN(a)||isNaN(s)?null:[a,s]}#z(e){if(!e)return null;const n=e.match(/^(\w+)/i);if(!n)return null;const s=n[1].toUpperCase();try{if(s==="POLYGON"||s==="MULTIPOLYGON")return this.#q(e,s);if(s==="LINESTRING"||s==="MULTILINESTRING")return this.#H(e,s);if(s==="POINT"){const a=this.#P(e);return a?{type:"Point",coordinates:[a[1],a[0]]}:null}}catch{}return null}#q(e,n){const s=[],a=/\(\s*([\d.\s,-]+)\s*\)/g;let l;for(;(l=a.exec(e))!==null;){const h=l[1].split(",").map(c=>{const[u,m]=c.trim().split(/\s+/).map(parseFloat);return[m,u]});h.length>0&&s.push(h)}return s.length===0?null:{type:n==="MULTIPOLYGON"?"MultiPolygon":"Polygon",coordinates:s}}#H(e,n){const s=[];if(n==="LINESTRING"){const a=e.match(/LINESTRING\s*\(\s*([\d.\s,-]+)\s*\)/i);if(a){const l=a[1].split(",").map(h=>{const[c,u]=h.trim().split(/\s+/).map(parseFloat);return[u,c]});s.push(l)}}else{const a=/\(\s*([\d.\s,-]+)\s*\)/g;let l;for(;(l=a.exec(e))!==null;){const h=l[1].split(",").map(c=>{const[u,m]=c.trim().split(/\s+/).map(parseFloat);return[m,u]});h.length>0&&s.push(h)}}return s.length===0?null:{type:n==="MULTILINESTRING"?"MultiLineString":"LineString",coordinates:s}}#N(e){if(e.type==="address"&&e.subtype){const n=e.subtype.toLowerCase();if(n==="provincie")return 5e4;if(n==="gemeente")return 2e4;if(n==="woonplaats")return 15e3;if(n==="wijk"||n==="buurt")return 3e3;if(n==="weg")return 5e3}return this.radius}#G(e=!1){const n=this.#t.querySelector(".search__listbox"),s=this.#t.querySelector(".search__input");if(this.#r.length===0){n.innerHTML=`
			...
			`,this.#l=-1;return}let a="";e&&(a='<li class="search__no-results" part="no-results" role="option" aria-disabled="true">Zoeken in adressen niet beschikbaar</li>');const l=this.#r.filter(v=>v.type==="feature"||v.type==="marker"),h=this.#r.filter(v=>v.type==="address"),c=l.length>0&&h.length>0,u=(v,g)=>{const _=g===this.#l;return`
				<li
					id="search-option-${g}"
			...
			`};let m="";c?(m+='<li class="search__group-label" part="group-label" role="presentation">Op de kaart</li>',m+=l.map(v=>u(v,this.#r.indexOf(v))).join(""),m+='<li class="search__group-label" part="group-label" role="presentation">Adressen</li>',m+=h.map(v=>u(v,this.#r.indexOf(v))).join("")):m+=this.#r.map((v,g)=>u(v,g)).join(""),n.innerHTML=m+a,this.#l>=0?s.setAttribute("aria-activedescendant",`search-option-${this.#l}`):s.removeAttribute("aria-activedescendant")}#$(e){switch(e.type){case"address":return e.subtype||"Adres";case"feature":return e.subtype||"Object";case"marker":return"Marker";default:return""}}#X(e){switch(e.type){case"address":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-map-pin"/></svg>';case"feature":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-hamburger"/></svg>';case"marker":return'<svg aria-hidden="true" width="20" height="20"><use href="#icon-map-pin"/></svg>';default:return""}}#W(){const e=this.#t.querySelector(".search__listbox"),n=this.#t.querySelector(".search__input"),s=this.#t.querySelector(".search__form");e.hidden=!1,n.setAttribute("aria-expanded","true"),s.dataset.open="true"}#k(){const e=this.#t.querySelector(".search__listbox"),n=this.#t.querySelector(".search__input"),s=this.#t.querySelector(".search__form");e.hidden=!0,n.setAttribute("aria-expanded","false"),n.removeAttribute("aria-activedescendant"),this.#l=-1,delete s.dataset.open}#F(e){const n=this.#r.length-1;if(n<0)return;this.#l+=e,this.#l<0?this.#l=n:this.#l>n&&(this.#l=0),this.#G();const s=this.#t.querySelector(`#search-option-${this.#l}`);s&&s.scrollIntoView({block:"nearest"})}#Z(e){const n=this.#r[e];if(!n)return;const s=this.#t.querySelector(".search__input"),a=this.#t.querySelector(".search__clear");s.value=n.label,a&&(a.hidden=!n.label),this.#k(),this.#Q(n),this.#J(n),this.dispatchEvent(new CustomEvent("dm-search:select",{bubbles:!0,composed:!0,detail:{type:n.type,label:n.label,coordinates:n.coordinates,bounds:n.bounds,feature:n.feature,marker:n.marker,raw:n.raw}}))}#Q(e){const n=this.#n.map;if(!n)return;this.#C();const s=e.geometry?.type||"";s.includes("Polygon")||s.includes("Line")?this.#O(e):e.coordinates&&this.#j(e.coordinates);let a;if(this.#o?.getBounds)a=this.#o.getBounds();else if(e.bounds)a=e.bounds;else if(e.coordinates){const l=O.latLng(e.coordinates[0],e.coordinates[1]),h=this.#N(e);a=l.toBounds(h)}a&&(n.fitBounds(a,{padding:[40,40]}),this.dispatchEvent(new CustomEvent("dm-search:zoom",{bubbles:!0,composed:!0,detail:{bounds:a,result:e}})))}#O(e){const n=this.#n.map;if(!n||!e.geometry)return;const s=e.geometry,a=s.type==="LineString"||s.type==="MultiLineString",l=this.#V(),h={color:l,weight:a?4:2,opacity:.8,fillColor:l,fillOpacity:a?0:.15};s.type==="Polygon"||s.type==="MultiPolygon"?this.#o=O.polygon(s.coordinates,h):s.type==="LineString"?this.#o=O.polyline(s.coordinates[0],h):s.type==="MultiLineString"&&(this.#o=O.polyline(s.coordinates,h)),this.#o&&this.#o.addTo(n)}#j(e){const n=this.#n?.map;!n||!e||(this.#o=O.circleMarker(e,{radius:9,color:"#fff",weight:2,fillColor:this.#V(),fillOpacity:1,opacity:1}).addTo(n))}#V(){const e=getComputedStyle(this);return e.getPropertyValue("--draad-map-components-search-highlight-color").trim()||e.getPropertyValue("--draad-map-components-color-primary").trim()||"#268641"}#J(e){if(e.type==="marker"&&e.marker)e.marker.dispatchEvent(new CustomEvent("dm-marker:click",{bubbles:!0,composed:!0,detail:{marker:e.marker.marker??null,targetId:e.marker.target}}));else if(e.type==="feature"&&e.source){const n=e.source.tagName==="DM-WFS"?"wfs":"geojson";e.source.dispatchEvent(new CustomEvent(`dm-${n}:feature-click`,{bubbles:!0,composed:!0,detail:{feature:e.feature,layer:e.layer,latlng:null}}))}}#C(){this.#o&&this.#n?.map&&(this.#n.map.removeLayer(this.#o),this.#o=null)}#D(e){const n=this.#t.querySelector(".search__button");e?(n.classList.add("search__button--loading"),n.setAttribute("aria-busy","true")):(n.classList.remove("search__button--loading"),n.removeAttribute("aria-busy"))}#B(e){const n=this.#t.querySelector('[role="status"]');n&&(n.textContent="",setTimeout(()=>{n.textContent=e},100))}#T(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}#K(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}disconnectedCallback(){this.#i&&(this.#i.abort(),this.#i=null),this.#a&&(clearTimeout(this.#a),this.#a=null),this.#s&&(clearTimeout(this.#s),this.#s=null),this.#C(),this.#n=null,this.#r=[]}}customElements.define("dm-search",tu);const eu=`/* dm-infowindow - Card panel styling */
:host {
	display: flex;
	flex-direction: column;
	max-block-size: 100%;
...
`;class iu extends HTMLElement{#t;#n=null;#e=null;#i=null;#a=null;#s=null;#l=null;#r=null;#o=null;#h=null;#u=!1;#d;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#d=this.#c()}#c(){const e=new CSSStyleSheet;e.replaceSync(eu),this.#t.adoptedStyleSheets=[e]}static get observedAttributes(){return["for","feature-id","trigger"]}get for(){return this.getAttribute("for")}set for(e){this.setAttribute("for",e)}get featureId(){return this.getAttribute("feature-id")}set featureId(e){this.setAttribute("feature-id",e)}get trigger(){return this.getAttribute("trigger")||"click"}set trigger(e){this.setAttribute("trigger",e)}get isOpen(){return this.#u}connectedCallback(){const e=this.closest("dm-map");e&&(this.#n=e,this.setAttribute("slot","top-right"),this.#v(),this.setAttribute("hidden",""),Ut(this.#t),e.map?this.#f():e.addEventListener("dm-map:ready",()=>{this.#f()},{once:!0}))}async#f(){await this.#d,this.#y(),this.#s=e=>this.#_(e),this.#n.addEventListener("dm-infowindow:open",this.#s)}#_(e){e.target!==this&&this.isOpen&&this.close()}#v(){this.#t.innerHTML=`
		...
		`,this.#t.querySelector(".infowindow__close").addEventListener("click",()=>this.close())}#y(){const e=this.for;e&&(this.#e=document.getElementById(e),this.#e?this.#p():this.#A(e))}#A(e){this.#r&&this.#r.disconnect(),this.#r=new MutationObserver((n,s)=>{const a=document.getElementById(e);a&&(this.#e=a,this.#p(),s.disconnect(),this.#r=null)}),this.#r.observe(this.#n,{childList:!0,subtree:!0})}#p(){const e=this.trigger,n=this.#e.tagName.toLowerCase();n==="dm-marker"?this.#L(e):(n==="dm-geojson"||n==="dm-wfs")&&this.#M(e,n)}#g(){if(this.#e&&(this.#i&&(this.#e.removeEventListener("dm-marker:click",this.#i),this.#i=null),this.#a)){const e=this.#e.tagName.toLowerCase().replace("dm-","");this.#e.removeEventListener(`dm-${e}:feature-click`,this.#a),this.#a=null}}#L(e){e==="click"&&(this.#i=n=>this.#w(n),this.#e.addEventListener("dm-marker:click",this.#i))}#M(e,n){const a=`dm-${n.replace("dm-","")}:feature-click`;e==="click"&&(this.#a=l=>this.#b(l),this.#e.addEventListener(a,this.#a))}#w(e){if(this.#h=null,this.isOpen)this.close();else{const n=this.#m();this.#G(n),this.open()}}#b(e){const{feature:n}=e.detail;if(!this.#S(n))return;this.#h=n;const s=this.#E(n);this.#G(s),this.#q(this,n),this.open()}#S(e){const n=this.featureId;return!n||n==="*"||e.id!==void 0&&String(e.id)===n||e.properties?.id!==void 0&&String(e.properties.id)===n}#m(){const e=document.createElement("div"),n=this.querySelector("template");if(n)e.appendChild(n.content.cloneNode(!0));else for(const s of this.childNodes)s.nodeType===Node.ELEMENT_NODE&&s.hasAttribute("slot")||e.appendChild(s.cloneNode(!0));return this.#P(e),e}#E(e){const n=document.createElement("div"),s=this.querySelector("template");if(s){const a=s.content.cloneNode(!0);n.appendChild(a),this.#q(n,e),this.#H(n)}else if(this.#e&&this.#I()){const a=this.#z(e);a&&n.appendChild(a)}else for(const a of this.childNodes)a.nodeType===Node.ELEMENT_NODE&&a.hasAttribute("slot")||n.appendChild(a.cloneNode(!0));return this.#P(n),n}#I(){return this.#e?.getAttribute("infowindow-properties")}#P(e){const n=a=>{const l=document.createElementNS("http://www.w3.org/2000/svg","svg");l.setAttribute("aria-hidden","true");const h=document.createElementNS("http://www.w3.org/2000/svg","use");return h.setAttribute("href",`#${a}`),l.appendChild(h),l},s=a=>{const l=a.getAttribute("href");if(!l)return!1;try{return new URL(l,window.location.href).origin!==window.location.origin}catch{return!1}};e.querySelectorAll(".address, address").forEach(a=>{a.setAttribute("part","address"),a.prepend(n("icon-map-pin"))}),e.querySelectorAll(".action, a.button, button.action").forEach(a=>{a.setAttribute("part","action"),s(a)&&a.prepend(n("icon-external-link"))})}#z(e){const n=this.#e.getAttribute("infowindow-properties"),s=this.#e.getAttribute("infowindow-labels");if(!n)return null;const a=n.split(",").map(c=>c.trim()),l=s?s.split(",").map(c=>c.trim()):a,h=document.createElement("dl");h.className="dm-infowindow-data",h.setAttribute("part","data-table");for(let c=0;c<a.length;c++){const u=e.properties?.[a[c]];if(u==null||u==="")continue;const m=document.createElement("dt");m.textContent=l[c]||a[c],h.appendChild(m);const v=document.createElement("dd");v.innerHTML=u!==null&&typeof u=="object"?JSON.stringify(u):String(u),h.appendChild(v)}return h.children.length>0?h:null}#q(e,n){const s=new Set(["https:","http:","mailto:","tel:"]),a=(l,h)=>{l.style.display=h?"":"none"};e.querySelectorAll("[data-field]").forEach(l=>{const h=this.#N(n,l.getAttribute("data-field")),c=h!=null?String(h):"";l.textContent=c,a(l,c!=="")}),e.querySelectorAll("[data-href]").forEach(l=>{const h=this.#N(n,l.getAttribute("data-href"));let c=!1;if(h!=null)try{const u=new URL(String(h),window.location.origin);s.has(u.protocol)?(l.setAttribute("href",String(h)),c=!0):l.removeAttribute("href")}catch{l.removeAttribute("href")}else l.removeAttribute("href");a(l,c)}),e.querySelectorAll("[data-src]").forEach(l=>{const h=this.#N(n,l.getAttribute("data-src"));let c=!1;if(h!=null)try{const u=new URL(String(h),window.location.origin);u.protocol==="https:"||u.protocol==="http:"?(l.setAttribute("src",String(h)),c=!0):l.removeAttribute("src")}catch{l.removeAttribute("src")}else l.removeAttribute("src");a(l,c)})}#H(e){e.querySelectorAll("dl.dm-infowindow-data").forEach(n=>{const s=Array.from(n.children);let a=!1;s.forEach((l,h)=>{if(l.tagName!=="DD")return;if(l.style.display==="none"||l.textContent===""){l.style.display="none";const u=s[h-1];u&&u.tagName==="DT"&&(u.style.display="none")}else a=!0}),a||(n.style.display="none")})}#N(e,n){if(!n.startsWith("properties.")&&n!=="id"&&n!=="properties")return;const s=new Set(["__proto__","constructor","prototype"]),a=n.split(".");let l=e;for(const h of a){if(l==null||s.has(h)||!Object.prototype.hasOwnProperty.call(l,h))return;l=l[h]}return l}#G(e){const n=this.#t.querySelector(".infowindow__content");for(n.innerHTML="";e.firstChild;)n.appendChild(e.firstChild);n.querySelectorAll("a").forEach(s=>s.setAttribute("part","link"))}open(){this.#t.querySelector(".infowindow").removeAttribute("hidden"),this.removeAttribute("hidden"),this.#u=!0,this.#$(),this.dispatchEvent(new CustomEvent("dm-infowindow:open",{bubbles:!0,composed:!0,detail:{target:this.#e,feature:this.#h}}))}close(){this.#t.querySelector(".infowindow").setAttribute("hidden",""),this.setAttribute("hidden",""),this.#u=!1,this.#l&&(document.removeEventListener("keydown",this.#l),this.#l=null),this.#e?.tagName.toLowerCase()==="dm-marker"&&(this.#e.active=!1),this.#o&&typeof this.#o.focus=="function"&&this.#o.focus(),this.#o=null,this.dispatchEvent(new CustomEvent("dm-infowindow:close",{bubbles:!0,composed:!0,detail:{target:this.#e}}))}toggle(){this.isOpen?this.close():this.open()}#$(){const e=this.#t.querySelector(".infowindow");this.#o=document.activeElement,e.setAttribute("tabindex","-1"),e.focus(),this.#l=n=>{n.key==="Escape"&&this.isOpen&&this.close()},document.addEventListener("keydown",this.#l)}attributeChangedCallback(e,n,s){if(!(n===s||!this.#n))switch(e){case"for":this.#g(),this.#e=null,this.#y();break;case"trigger":this.#g(),this.#e&&this.#p();break}}disconnectedCallback(){this.#g(),this.#s&&(this.#n?.removeEventListener("dm-infowindow:open",this.#s),this.#s=null),this.#l&&(document.removeEventListener("keydown",this.#l),this.#l=null),this.#r&&(this.#r.disconnect(),this.#r=null),this.#n=null,this.#e=null,this.#o=null,this.#h=null}}customElements.define("dm-infowindow",iu);const nu=`.icon {
	display: block;
	inline-size: 1em;
	block-size: 1em;
	fill: currentColor;
...
.legend__item-description p {
	margin: 0;
}`;class ru extends HTMLElement{#t;#n=!1;#e=[];#i=new Set;#a;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#a=this.#s()}#s(){const e=new CSSStyleSheet;e.replaceSync(nu),this.#t.adoptedStyleSheets=[e]}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#l(e):e.addEventListener("dm-map:ready",()=>{this.#l(e)},{once:!0}))}async#l(e){await this.#a,this.#e=this.#o(e),this.#u(),Ut(this.#t),this.#d()}#r(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}#o(e){const n=[];return e.querySelectorAll(":scope > dm-layer[label]").forEach(l=>{n.push({element:l,label:l.label,type:"layer",layerType:this.#h(l),description:l.getAttribute("description")})}),[":scope > dm-marker[label]",":scope > dm-geojson[label]",":scope > dm-wfs[label]",":scope > dm-wms[label]"].forEach(l=>{e.querySelectorAll(l).forEach(c=>{const u=c.tagName.toLowerCase().replace("dm-","");n.push({element:c,label:c.label,type:u,layerType:this.#h(c),description:c.getAttribute("description")})})}),n}#h(e){const n=e.tagName.toLowerCase();if(n==="dm-marker")return"point";if(n==="dm-wms")return"raster";const s=e.getAttribute("geometry-type");if(s){if(s.toLowerCase().includes("point")||s.toLowerCase().includes("marker"))return"point";if(s.toLowerCase().includes("line"))return"polygon"}return"polygon"}#u(){const e=s=>s==="point"?'<svg width="16" height="16" aria-hidden="true"><use href="#icon-map-pin"/></svg>':s==="raster"?'<svg width="16" height="16" aria-hidden="true"><use href="#icon-grid"/></svg>':"",n=this.#e.map((s,a)=>{const l=s.element.hidden?"":"checked",h=this.#r(s.label),c=s.description?this.#r(s.description):"",u=!!s.description,m=this.#i.has(a),v=`legend-layer-${a}`;return`
...
`;class cu extends HTMLElement{#t;#n=null;#e=[];#i={};#a={};#s=new Map;#l=new WeakMap;#r=new Map;#o=new Map;#h;#u=!1;#d=!1;#c=null;#f=null;#_=null;#v=!1;#y=null;constructor(){super(),this.#t=this.attachShadow({mode:"open"}),this.#h=this.#A()}#A(){const e=new CSSStyleSheet;e.replaceSync(hu),this.#t.adoptedStyleSheets=[e]}get#p(){const e=this.getAttribute("variant");return e==="dropdown"||e==="sidebar"?e:"dialog"}get#g(){return(this.getAttribute("submit")||"auto")==="auto"}get#L(){return this.hasAttribute("collapsible")}connectedCallback(){const e=this.closest("dm-map");e&&(e.map?this.#M(e):e.addEventListener("dm-map:ready",()=>{this.#M(e)},{once:!0}))}async#M(e){if(this.#n=e,await this.#h,this.#w(),this.#b(),this.#H(),Ut(this.#t),this.#V(),this.#k(),this.#C(),this.hasAttribute("open")&&this.#p==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__toggle");s?.classList.add("filter__panel--open"),a?.setAttribute("aria-expanded","true"),this.#d=!0}this.#tt()&&(this.#u=!0,this.#a=JSON.parse(JSON.stringify(this.#i))),this.#y=()=>this.#tt(!0),window.addEventListener("popstate",this.#y)}#w(){const e=["dm-geojson[filter-property], dm-geojson[filter-properties]","dm-wfs[filter-property], dm-wfs[filter-properties]","dm-layer[filter-property], dm-layer[filter-properties]"];this.#e=[],e.forEach(n=>{this.#n.querySelectorAll(n).forEach(s=>{const a=s.getAttribute("filter-properties"),l=s.getAttribute("filter-labels"),h=s.getAttribute("filter-property"),c=s.getAttribute("filter-label"),u=s.getAttribute("filter-types"),m=s.getAttribute("filter-collapsed");let v=[],g=[],_=[],w=[];if(a){for(v=a.split(",").map(E=>E.trim()).filter(Boolean),l&&(g=l.split(",").map(E=>E.trim()).filter(Boolean)),u&&(_=u.split(",").map(E=>E.trim()).filter(Boolean)),m&&(w=m.split(",").map(E=>E.trim()).filter(Boolean));g.length<v.length;)g.push(v[g.length]);for(;_.length<v.length;)_.push("auto");for(;w.length<v.length;)w.push("false")}else h&&(v=[h],g=[c||h],_=[u||"auto"],w=[m||"false"]);if(v.length===0)return;const x=s.id||`filter-source-${this.#e.length}`,S=s.getAttribute("filter-key");if(v.forEach((E,C)=>{this.#e.push({element:s,label:g[C],property:E,groupKey:S||E,type:s.tagName.toLowerCase(),id:x,filterType:_[C]||"auto",collapsed:w[C]==="true"})}),!this.#l.has(s))if(this.#l.set(s,!0),s.data)v.forEach(E=>{this.#m(s,E)});else{const E=s.tagName==="DM-GEOJSON"?"dm-geojson:ready":s.tagName==="DM-WFS"?"dm-wfs:ready":"dm-layer:ready";let C=!1;s.addEventListener(E,()=>{C||(C=!0,v.forEach(k=>{this.#m(s,k)}),this.#b(),this.#k(),this.#C())},{once:!0}),s.data&&!C&&(C=!0,v.forEach(k=>{this.#m(s,k)}))}})})}#b(){this.#o=new Map,this.#e.forEach(e=>{this.#o.has(e.groupKey)||this.#o.set(e.groupKey,{label:e.label,filterType:e.filterType,collapsed:e.collapsed,sources:[]});const n=this.#o.get(e.groupKey);n.sources.find(a=>a.element===e.element&&a.property===e.property)||n.sources.push({element:e.element,id:e.id,property:e.property})})}#S(e,n){const s=new Set;if(n.sources.forEach(({id:l,property:h})=>{const c=`${l}:${h}`,u=this.#s.get(c);u&&u.values.forEach(m=>s.add(m))}),s.size===0)return null;const a=Array.from(s).sort((l,h)=>typeof l=="number"&&typeof h=="number"?l-h:String(l).localeCompare(String(h)));return{values:a,valueType:this.#P(a)}}#m(e,n){const s=this.#E(e,n),a=this.#P(s),l=this.#e.find(c=>c.element===e&&c.property===n);if(!l)return;const h=`${l.id}:${n}`;this.#s.set(h,{values:s,valueType:a}),this.#u&&Object.keys(this.#i).length>0&&this.#T(),this.#Z()&&(this.#k(),this.#C())}#E(e,n){if(!e.data?.features)return[];const s=new Set;return e.data.features.forEach(a=>{const l=a.properties?.[n];if(l!=null){const h=String(l);h.includes(",")?h.split(",").forEach(c=>{const u=c.trim();u&&s.add(u)}):s.add(l)}}),Array.from(s).sort((a,l)=>typeof a=="number"&&typeof l=="number"?a-l:String(a).localeCompare(String(l)))}#I(e,n){const s=new Map;return n.sources.forEach(({element:a,property:l})=>{a.data?.features&&a.data.features.forEach(h=>{const c=h.properties?.[l];if(c==null)return;const u=String(c);(u.includes(",")?u.split(",").map(v=>v.trim()).filter(Boolean):[u]).forEach(v=>{s.set(v,(s.get(v)||0)+1)})})}),s}#P(e){if(e.length===0)return{isNumeric:!1};if(e.length===1)return{isNumeric:!1,single:!0};if(e.every(s=>typeof s=="number"?!0:typeof s=="string"?!isNaN(Number(s))&&s.trim()!=="":!1)){const s=e.map(a=>Number(a));return{isNumeric:!0,min:Math.min(...s),max:Math.max(...s)}}return{isNumeric:!1}}#z(e,n){const s=this.#o.get(e)?.filterType;return s&&s!=="auto"?s==="range"&&!n.valueType.isNumeric?this.#q(n):s:this.#q(n)}#q({values:e,valueType:n}){return n.isNumeric&&!n.single?"range":e.length<=20?"checkbox":"dropdown"}#H(){const e=this.#p;e==="dropdown"?this.#X():e==="sidebar"?this.#W():this.#$()}#N(){return`
		...
		`}#$(){const e=this.getAttribute("label")||"Filters",n=this.#g?" filter--auto-submit":"";this.#t.innerHTML=`
		...
		`}#k(){if(!this.#v){this.#v=!0;try{const e=this.#t.querySelector(".filter__content");if(!e)return;if(this.#o.size===0){e.innerHTML='<p class="filter__empty">Geen filterbare gegevensbronnen gevonden.</p>';return}const n=[];this.#o.forEach((s,a)=>{let l=this.#S(a,s);if(l||(s.sources.forEach(({element:k,property:I})=>{k.data&&this.#m(k,I)}),l=this.#S(a,s)),!l||l.values.length===0){n.push(`
						<div class="filter__section" part="section" data-property="${this.#x(a)}">
							<h3 class="filter__section-title" part="section-title">${this.#R(s.label)}</h3>
							<p class="filter__loading">Loading data...</p>
						</div>
					`);return}const{values:h,valueType:c}=l,u=this.#z(a,l),m=s.collapsed,v=m?"false":"true",g=m?" hidden":"";if(u==="single"||c.single){n.push(`
						<div class="filter__section" part="section" data-property="${this.#x(a)}">
							${this.#F(s.label,a,v,g,`
								<p class="filter__single">Value: ${this.#R(String(h[0]))}</p>
							`)}
						</div>
					`);return}if(u==="range"){const k=this.#i[a]?.min??c.min,I=this.#i[a]?.max??c.max,B=`
					...
					`);return}if(u==="checkbox"){const k=s.sources.reduce((F,{element:J})=>F||this.#lt(J),null),I=this.#I(a,s),B=h.map(F=>{const J=this.#rt(a,F)?"checked":"",ot=this.#x(String(F)),et=this.#R(String(F)),Mt=I.get(String(F))||0,Q=Mt>0?` (${Mt})`:"",ht=k?.[F]?this.#ht(k[F]):this.#U();return`
						...
						<fieldset class="filter__fieldset">
							<legend class="visually-hidden">${this.#R(s.label)}</legend>
							${B}
						</fieldset>`;n.push(`
						<div class="filter__section" part="section" data-property="${this.#x(a)}">
							${this.#F(s.label,a,v,g,H)}
						</div>
					`);return}const _=this.#i[a]||[],w=_.map(k=>{const I=this.#x(String(k)),B=this.#R(String(k));return`
					...
					`}).join(""),x=h.filter(k=>!_.includes(String(k))).map(k=>{const I=this.#x(String(k)),B=this.#R(String(k));return`
				...
				`)}),e.innerHTML=n.join("")}finally{this.#v=!1}}}#F(e,n,s,a,l){if(this.#L){const h=`filter-section-${this.#x(n)}`;return`
				<button type="button" part="section-toggle" class="filter__section-toggle" aria-expanded="${s}" aria-controls="${h}">
					<span class="filter__section-toggle-text">${this.#R(e)}</span>
			...
			<h3 class="filter__section-title" part="section-title">${this.#R(e)}</h3>
			${l}`}#Z(){return this.#p==="dialog"?this.#t.querySelector("dialog")?.open??!1:this.#d}#Q(){const e=this.#p,n=this.#t.querySelector(".filter__toggle");if(this.#k(),this.#C(),e==="dialog")this.#t.querySelector("dialog").showModal(),n.setAttribute("aria-expanded","true");else if(e==="dropdown"){const s=this.#t.querySelector(".filter__panel--dropdown"),a=()=>{const l=n.getBoundingClientRect(),h=this.closest("dm-map"),c=h?h.getBoundingClientRect():null;s.style.top=`${l.bottom+4}px`,c?(s.style.left=`${c.left}px`,s.style.right=`${window.innerWidth-c.right}px`,s.style.inlineSize="auto"):(s.style.left="",s.style.right=`${window.innerWidth-l.right}px`,s.style.inlineSize="")};a(),s.classList.add("filter__panel--open"),n.setAttribute("aria-expanded","true"),this.#d=!0,this.#f=a,window.addEventListener("scroll",this.#f,{passive:!0,capture:!0}),this.#c=l=>{l.composedPath().some(h=>h===s||h===n)||this.#O()},requestAnimationFrame(()=>{document.addEventListener("click",this.#c,!0)}),this.#t.addEventListener("keydown",this.#j)}else if(e==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__overlay");s.classList.add("filter__panel--open"),a.classList.add("filter__overlay--visible"),n.setAttribute("aria-expanded","true"),this.#d=!0,this.#t.addEventListener("keydown",this.#j)}}#O(){const e=this.#p,n=this.#t.querySelector(".filter__toggle");if(e==="dialog")this.#t.querySelector("dialog").close();else if(e==="dropdown")this.#t.querySelector(".filter__panel--dropdown")?.classList.remove("filter__panel--open"),this.#d=!1,this.#f&&(window.removeEventListener("scroll",this.#f,{capture:!0}),this.#f=null),this.#c&&(document.removeEventListener("click",this.#c,!0),this.#c=null),this.#t.removeEventListener("keydown",this.#j);else if(e==="sidebar"){const s=this.#t.querySelector(".filter__panel--sidebar"),a=this.#t.querySelector(".filter__overlay");s?.classList.remove("filter__panel--open"),a?.classList.remove("filter__overlay--visible"),this.#d=!1,this.#t.removeEventListener("keydown",this.#j)}n.setAttribute("aria-expanded","false"),n.focus()}#j=e=>{e.key==="Escape"&&this.#O()};#V(){this.#t.querySelector(".filter__toggle").addEventListener("click",()=>{this.#Z()?this.#O():this.#Q()});const e=this.#t.querySelector('slot[name="header"]');if(e){const n=()=>{const s=e.assignedNodes().length>0;e.closest(".filter__header-text")?.classList.toggle("filter__header-text--empty",!s)};e.addEventListener("slotchange",n),n()}if(this.#t.querySelector(".filter__apply").addEventListener("click",()=>{this.#D(),this.#T(),this.#p==="dialog"&&this.#O()}),this.#t.querySelector(".filter__reset").addEventListener("click",()=>this.#K()),this.#t.querySelector(".filter__close").addEventListener("click",()=>this.#O()),this.#p==="dialog"){const n=this.#t.querySelector("dialog");n.addEventListener("close",()=>{const s=this.#t.querySelector(".filter__toggle");s.setAttribute("aria-expanded","false"),s.focus()}),n.addEventListener("click",s=>{s.target===n&&this.#O()})}this.#p==="sidebar"&&this.#t.querySelector(".filter__overlay")?.addEventListener("click",()=>this.#O())}#J(e){const n=e.querySelector(".filter__custom-select-display"),s=e.querySelector(".filter__custom-select-dropdown");if(!n||!s)return;const a=n.getBoundingClientRect(),l=4,h=240,c=window.innerHeight,u=c-a.bottom-l,m=a.top-l,v=u<160&&m>u,g=v?m:u;s.style.left=`${a.left}px`,s.style.width=`${a.width}px`,s.style.maxBlockSize=`${Math.max(120,Math.min(h,g))}px`,v?(s.style.top="auto",s.style.bottom=`${c-a.top+l}px`):(s.style.bottom="auto",s.style.top=`${a.bottom+l}px`)}#C(){const e=this.#t.querySelector(".filter__content");if(!e)return;this.#_?.abort(),this.#_=new AbortController;const{signal:n}=this.#_,s=this.#g;e.querySelectorAll(".filter__section-toggle").forEach(a=>{a.addEventListener("click",()=>{const l=a.getAttribute("aria-expanded")==="true",h=a.getAttribute("aria-controls"),c=this.#t.getElementById(h);c&&(a.setAttribute("aria-expanded",l?"false":"true"),l?c.setAttribute("hidden",""):c.removeAttribute("hidden"))},{signal:n})}),e.querySelectorAll(".filter__checkbox").forEach(a=>{a.addEventListener("change",()=>{this.#D(),s?this.#T():this.#B()},{signal:n})}),e.querySelectorAll(".filter__custom-select").forEach(a=>{a.addEventListener("click",l=>{if(l.target.closest(".filter__chip-remove"))return;const h=a.getAttribute("aria-expanded")==="true";if(e.querySelectorAll(".filter__custom-select").forEach(c=>{c.setAttribute("aria-expanded","false")}),a.setAttribute("aria-expanded",h?"false":"true"),!h){this.#J(a);const c=a.querySelector(".filter__option");c&&setTimeout(()=>c.focus(),0)}},{signal:n}),a.addEventListener("keydown",l=>{if(l.key==="Enter"||l.key===" "){l.preventDefault();const h=a.getAttribute("aria-expanded")==="true";if(a.setAttribute("aria-expanded",h?"false":"true"),!h){this.#J(a);const c=a.querySelector(".filter__option");c&&setTimeout(()=>c.focus(),0)}}else if(l.key==="Escape")a.setAttribute("aria-expanded","false"),a.focus();else if(l.key==="ArrowDown"){l.preventDefault();const h=[...a.querySelectorAll(".filter__option")],c=h.indexOf(this.#t.activeElement);(h[c+1]||h[0])?.focus()}else if(l.key==="ArrowUp"){l.preventDefault();const h=[...a.querySelectorAll(".filter__option")],c=h.indexOf(this.#t.activeElement);(h[c-1]||h[h.length-1])?.focus()}},{signal:n})}),e.querySelectorAll(".filter__option").forEach(a=>{a.addEventListener("click",l=>{const c=l.target.closest(".filter__section").dataset.property,u=l.target.dataset.value;this.#i[c]||(this.#i[c]=[]),this.#i[c].push(u),this.#k(),this.#C(),s?this.#T():this.#B()},{signal:n}),a.addEventListener("keydown",l=>{l.key==="Enter"&&(l.preventDefault(),a.click())},{signal:n})}),e.querySelectorAll(".filter__chip-remove").forEach(a=>{a.addEventListener("click",l=>{l.stopPropagation();const h=l.target.closest(".filter__chip"),u=l.target.closest(".filter__section").dataset.property,m=h.dataset.value;this.#i[u]&&(this.#i[u]=this.#i[u].filter(v=>v!==m),this.#i[u].length===0&&delete this.#i[u]),this.#k(),this.#C(),s?this.#T():this.#B()},{signal:n})}),this.#t.addEventListener("click",a=>{a.composedPath().some(l=>l?.classList?.contains("filter__custom-select"))||e.querySelectorAll(".filter__custom-select").forEach(l=>{l.setAttribute("aria-expanded","false")})},{signal:n}),e.querySelectorAll(".filter__range-input").forEach(a=>{const l=a.closest(".filter__section")?.dataset.property||"unknown",h=this.#it(()=>{this.#D(),s?this.#T():this.#B()},150,`range-${l}-${a.dataset.bound}`);a.addEventListener("input",c=>{const u=c.target.closest("label").querySelector("output");u&&(u.textContent=c.target.value),h()},{signal:n}),a.addEventListener("change",()=>{this.#D(),s?this.#T():this.#B()},{signal:n})})}#D(){const e={};this.#t.querySelectorAll(".filter__section").forEach(n=>{const s=n.dataset.property,a=n.querySelectorAll(".filter__checkbox");if(a.length>0){const c=Array.from(a).filter(u=>u.checked).map(u=>u.value);c.length>0&&(e[s]=c);return}const l=n.querySelectorAll(".filter__chip");if(l.length>0){const c=Array.from(l).map(u=>u.dataset.value);c.length>0&&(e[s]=c);return}if(n.querySelectorAll(".filter__range-input").length===2){const c=n.querySelector('[data-bound="min"]'),u=n.querySelector('[data-bound="max"]');c&&u&&(e[s]={min:Number(c.value),max:Number(u.value)})}}),this.#i=e,this.#u=!1}#B(){const e=this.#t.querySelector(".filter__apply");if(e){const n=JSON.stringify(this.#i)!==JSON.stringify(this.#a);e.classList.toggle("filter__apply--dirty",n)}}#T(){const e=new Map;this.#o.forEach((a,l)=>{const h=this.#i[l];a.sources.forEach(({element:c,property:u})=>{e.has(c)||e.set(c,{}),h!==void 0&&(e.get(c)[u]=h)})}),e.forEach((a,l)=>{l.dispatchEvent(new CustomEvent("dm-filter:apply",{bubbles:!1,detail:{filters:a}}))}),this.#a=JSON.parse(JSON.stringify(this.#i)),this.#Y();const n=this.#t.querySelector(".filter__apply");n&&n.classList.remove("filter__apply--dirty");const s=Object.keys(this.#i).length;this.#it(()=>{this.#nt(s>0?`${s} filter${s!==1?"s":""} actief`:"Alle filters gewist")},500,"announce")()}#K(){this.#i={},this.#u=!1;const e=new Set;this.#o.forEach(s=>{s.sources.forEach(({element:a})=>e.add(a))}),e.forEach(s=>{s.dispatchEvent(new CustomEvent("dm-filter:apply",{bubbles:!1,detail:{filters:{}}}))}),this.#a={},this.#Y(),this.#nt("Alle filters zijn gewist"),this.#k(),this.#C();const n=this.#t.querySelector(".filter__apply");n&&n.classList.remove("filter__apply--dirty")}#rt(e,n){const s=this.#i[e];return!s||!Array.isArray(s)?!1:s.includes(String(n))}#Y(){try{const e=new URL(window.location.href);Object.keys(this.#i).length===0?e.searchParams.delete("filters"):e.searchParams.set("filters",this.#st(this.#i)),window.history.pushState({},"",e)}catch{}}#tt(e=!1){try{const n=new URLSearchParams(window.location.search).get("filters");if(!n)return!1;const s=this.#at(n);return this.#i=s,e&&this.#T(),!0}catch{return!1}}#st(e){return btoa(encodeURIComponent(JSON.stringify(e)))}#at(e){if(e.length>1e4)return{};const n=decodeURIComponent(atob(e));return this.#ot(JSON.parse(n))}#ot(e){if(typeof e!="object"||e===null||Array.isArray(e))return{};const n={};for(const[s,a]of Object.entries(e))Array.isArray(a)&&a.every(l=>typeof l=="string")?n[s]=a:a&&typeof a=="object"&&!Array.isArray(a)&&typeof a.min=="number"&&typeof a.max=="number"&&(n[s]={min:a.min,max:a.max});return n}#et(e,n){const s=n-e;return s<=10?.1:s<=100?1:s<=1e3?10:100}#R(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}#x(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}#lt(e){const n=e.getAttribute("filter-icons");if(!n)return null;try{return JSON.parse(n)}catch{return null}}#U(){return`<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
			<circle cx="10" cy="10" r="7" stroke="currentColor" stroke-width="1.5" fill="none"/>
		</svg>`}#ht(e){if(!e||typeof e!="string")return this.#U();try{const n=new DOMParser().parseFromString(e,"image/svg+xml");if(n.querySelector("parsererror"))return this.#U();const s=n.querySelector("svg");return s?(s.querySelectorAll("script, foreignObject, object, embed, iframe").forEach(a=>a.remove()),s.querySelectorAll("*").forEach(a=>{for(const l of[...a.attributes])(l.name.startsWith("on")||l.name==="href"||l.name==="xlink:href"||l.name.toLowerCase().includes("javascript"))&&a.removeAttribute(l.name)}),s.querySelectorAll("[href], [xlink\\:href]").forEach(a=>{const l=a.getAttribute("href")||a.getAttribute("xlink:href");l&&l.toLowerCase().includes("javascript:")&&a.remove()}),s.outerHTML):this.#U()}catch{return this.#U()}}#it(e,n=150,s="default"){return(...a)=>{this.#r.has(s)&&clearTimeout(this.#r.get(s));const l=setTimeout(()=>{e.apply(this,a),this.#r.delete(s)},n);this.#r.set(s,l)}}#nt(e){const n=this.#t.querySelector('[role="status"]');n&&(n.textContent="",setTimeout(()=>{n.textContent=e},100))}get activeFilterCount(){return Object.keys(this.#i).length}getFilterState(){return JSON.parse(JSON.stringify(this.#i))}resetFilters(){this.#K()}exportFilters(){return JSON.stringify(this.#i)}disconnectedCallback(){this.#r.forEach(e=>clearTimeout(e)),this.#r.clear(),this.#_?.abort(),this.#_=null,this.#c&&(document.removeEventListener("click",this.#c,!0),this.#c=null),this.#y&&(window.removeEventListener("popstate",this.#y),this.#y=null),this.#e=[],this.#i={},this.#a={},this.#s.clear(),this.#o.clear(),this.#n=null}}return customElements.define("dm-filter",cu),Oe.ICON_SPRITE=jd,Oe.registerIcons=Ud,Oe.setIconSprite=Hd,Object.defineProperty(Oe,Symbol.toStringTag,{value:"Module"}),Oe})({});
